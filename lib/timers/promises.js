'use strict';
const { Timeout, Immediate, insert, } = require('internal/timers');
const { clearImmediate, clearInterval, clearTimeout, } = require('timers');
const { AbortError, codes: { ERR_ILLEGAL_CONSTRUCTOR, ERR_INVALID_ARG_TYPE, ERR_INVALID_THIS, }, } = require('internal/errors');
const { validateAbortSignal, validateBoolean, validateObject, } = require('internal/validators');
...
function setTimeout(after, value, options = kEmptyObject) {
    ...
    if (options == null || typeof options !== 'object') {
        return Promise.reject(new ERR_INVALID_ARG_TYPE('options', 'Object', options));
    }
    ...
    if (typeof ref !== 'boolean') {
        return Promise.reject(new ERR_INVALID_ARG_TYPE('options.ref', 'boolean', ref));
    }
    if (signal?.aborted) {
    ...
    const ret = new Promise((resolve, reject) => {
        ...
        if (signal) {
            ...
            signal.addEventListener('abort', oncancel, { [kResistStopPropagation]: true });
        }
    });
...
function setImmediate(value, options = kEmptyObject) {
    ...
    const ret = new Promise((resolve, reject) => {
        ...
        if (!ref)
            immediate.unref();
        if (signal) {
            oncancel = cancelListenerHandler.bind(immediate, clearImmediate, reject, signal);
            kResistStopPropagation ??= require('internal/event_target').kResistStopPropagation;
            signal.addEventListener('abort', oncancel, { [kResistStopPropagation]: true });
        }
    });
...
async function* setInterval(after, value, options = kEmptyObject) {
    ...
    try {
        ...
        if (signal) {
            ...
            kResistStopPropagation ??= require('internal/event_target').kResistStopPropagation;
            signal.addEventListener('abort', onCancel, { once: true, [kResistStopPropagation]: true });
        }
