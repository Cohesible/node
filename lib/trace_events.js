...
const { ERR_TRACE_EVENTS_CATEGORY_REQUIRED, ERR_TRACE_EVENTS_UNAVAILABLE, } = require('internal/errors').codes;
    ...
    enable() {
        if (!this.#enabled) {
            ...
            if (enabledTracingObjects.size > kMaxTracingCount) {
                process.emitWarning('Possible trace_events memory leak detected. There are more than ' +
