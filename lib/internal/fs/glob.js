...
const { validateFunction, validateObject, validateString, validateStringArray, } = require('internal/validators');
let minimatch;
...
class Cache {
    ...
    stat(path) {
        ...
        const promise = lstat(path).then(null, () => null);
    ...
    async readdir(path) {
        ...
        const promise = readdir(path, { __proto__: null, withFileTypes: true }).then(null, () => null);
...
class Pattern {
    ...
    get hasSeenSymlinks() {
        return Array.from(this.indexes).some((i) => !this.symlinks.has(i));
    }
    at(index) {
...
class Glob {
    ...
    constructor(pattern, options = kEmptyObject) {
        ...
        this.matchers = patterns.map((pattern) => new (lazyMinimatch().Minimatch)(pattern, {
            __proto__: null,
        ...
        this.#patterns = this.matchers.flatMap((matcher) => matcher.set.map((pattern, i) => new Pattern(pattern, matcher.globParts[i], new Set().add(0), new Set())));
    }
    globSync() {
        ...
        while (this.#queue.length > 0) {
            ...
            this.#subpatterns
                .forEach((patterns, path) => this.#queue.push({ __proto__: null, path, patterns }));
