...
const { AbortError, codes: { ERR_INVALID_ARG_VALUE, }, } = require('internal/errors');
...
const { basename: pathBasename, join: pathJoin, relative: pathRelative, resolve: pathResolve, } = require('path');
let internalSync;
        ...
        const promiseExecutor = signal == null ?
            (resolve) => {
                this.once('change', (eventType, filename) => {
                    resolve({ value: { eventType, filename } });
                });
            } : (resolve, reject) => {
            const onAbort = () => {
                this.close();
