...
const { codes: { ERR_DIR_CLOSED, ERR_DIR_CONCURRENT_OPERATION, ERR_MISSING_ARGS, }, } = require('internal/errors');
const { FSReqCallback } = binding;
const internalUtil = require('internal/util');
const { getDirent, getOptions, getValidatedPath, handleErrorFromBinding, } = require('internal/fs/utils');
const { validateFunction, validateUint32, } = require('internal/validators');
class Dir {
    #handle;
    #path;
    #bufferedEntries = [];
    ...
    constructor(handle, path, options) {
        ...
        this.#readPromisified = internalUtil.promisify(this.#readImpl).bind(this, false);
        this.#closePromisified = internalUtil.promisify(this.close).bind(this);
    }
    get path() {
        return this.#path;
    }
    ...
    #readImpl(maybeSync, callback) {
        ...
        this.#operationQueue = [];
        this.#handle.read(this.#options.encoding, this.#options.bufferSize, req);
    }
    processReadResult(path, result) {
        for (let i = 0; i < result.length; i += 2) {
            this.#bufferedEntries.push(getDirent(path, result[i], result[i + 1]));
        }
    }
    readSyncRecursive(dirent) {
        const path = pathModule.join(dirent.parentPath, dirent.name);
        const ctx = { path };
        const handle = dirBinding.opendir(pathModule.toNamespacedPath(path), this.#options.encoding, undefined, ctx);
        handleErrorFromBinding(ctx);
        const result = handle.read(this.#options.encoding, this.#options.bufferSize, undefined, ctx);
        if (result) {
    ...
    readSync() {
        ...
        const result = this.#handle.read(this.#options.encoding, this.#options.bufferSize, undefined, ctx);
        handleErrorFromBinding(ctx);
        if (result === null) {
            return result;
        }
...
function opendirSync(path, options) {
    ...
    const handle = dirBinding.opendirSync(pathModule.toNamespacedPath(path));
