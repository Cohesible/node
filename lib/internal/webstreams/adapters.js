...
const { ReadableStream, isReadableStream, } = require('internal/webstreams/readablestream');
const { WritableStream, isWritableStream, } = require('internal/webstreams/writablestream');
const { CountQueuingStrategy, } = require('internal/webstreams/queuingstrategies');
...
const { ErrnoException, codes: { ERR_INVALID_ARG_TYPE, ERR_INVALID_ARG_VALUE, ERR_INVALID_STATE, ERR_STREAM_PREMATURE_CLOSE, }, AbortError, } = require('internal/errors');
const { createDeferredPromise, kEmptyObject, normalizeEncoding, } = require('internal/util');
const { validateBoolean, validateFunction, validateObject, } = require('internal/validators');
const { WriteWrap, ShutdownWrap, kReadBytesOrError, kLastWriteWasAsync, streamBaseState, } = internalBinding('stream_wrap');
const finished = require('internal/streams/end-of-stream');
const { UV_EOF } = internalBinding('uv');
const encoder = new TextEncoder();
// Collect all negative (error) ZLIB codes and Z_NEED_DICT
const ZLIB_FAILURES = new Set([
    ...Object.entries(internalBinding('constants').zlib).map(({ 0: code, 1: value }) => (value < 0 ? code : null)).filter(Boolean),
    'Z_NEED_DICT',
]);
/**
...
function newStreamWritableFromWritableStream(writableStream, options = kEmptyObject) {
    ...
    const writable = new Writable({
        ...
        writev(chunks, callback) {
            ...
            writer.ready.then(() => {
                return Promise.all(chunks.map((data) => writer.write(data.chunk))).then(done, done);
            }, done);
        },
...
function newWritableStreamFromStreamBase(streamBase, strategy) {
    ...
    return new WritableStream({
        write(chunk, controller) {
            current = current !== undefined ? current.then(() => doWrite(chunk, controller), (error) => controller.error(error)) :
