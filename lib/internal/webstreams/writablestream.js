'use strict';
const { codes: { ERR_ILLEGAL_CONSTRUCTOR, ERR_INVALID_ARG_VALUE, ERR_INVALID_ARG_TYPE, ERR_INVALID_STATE, ERR_INVALID_THIS, }, } = require('internal/errors');
const { DOMException, } = internalBinding('messaging');
const { createDeferredPromise, customInspectSymbol: kInspect, kEmptyObject, kEnumerableProperty, SideEffectFreeRegExpPrototypeSymbolReplace, } = require('internal/util');
const { validateObject, kValidateObjectAllowObjects, kValidateObjectAllowObjectsAndNull, } = require('internal/validators');
const { MessageChannel, } = require('internal/worker/io');
const { kDeserialize, kTransfer, kTransferList, markTransferMode, } = require('internal/worker/js_transferable');
const { createPromiseCallback, customInspect, dequeueValue, enqueueValueWithSize, extractHighWaterMark, extractSizeAlgorithm, lazyTransfer, isBrandCheck, isPromisePending, peekQueueValue, resetQueue, setPromiseHandled, nonOpCancel, nonOpStart, nonOpWrite, kType, kState, } = require('internal/webstreams/util');
const { kIsClosedPromise, kControllerErrorFunction, } = require('internal/streams/utils');
...
class WritableStream {
    ...
    abort(reason = undefined) {
        ...
        if (isWritableStreamLocked(this)) {
            return Promise.reject(new ERR_INVALID_STATE.TypeError('WritableStream is locked'));
        }
    ...
    close() {
        ...
        if (writableStreamCloseQueuedOrInFlight(this)) {
            return Promise.reject(new ERR_INVALID_STATE.TypeError('Failure closing WritableStream'));
        }
    ...
    [kTransfer]() {
        ...
        const { readable, promise, } = lazyTransfer().newCrossRealmReadableStream(this, this[kState].transfer.port1);
...
class WritableStreamDefaultWriter {
    ...
    abort(reason = undefined) {
        if (!isWritableStreamDefaultWriter(this))
            return Promise.reject(new ERR_INVALID_THIS('WritableStreamDefaultWriter'));
        if (this[kState].stream === undefined) {
            return Promise.reject(new ERR_INVALID_STATE.TypeError('Writer is not bound to a WritableStream'));
        }
    ...
    close() {
        ...
        if (writableStreamCloseQueuedOrInFlight(stream)) {
            return Promise.reject(new ERR_INVALID_STATE.TypeError('Failure to close WritableStream'));
        }
...
function writableStreamClose(stream) {
    ...
    if (state === 'closed' || state === 'errored') {
        return Promise.reject(new ERR_INVALID_STATE.TypeError('WritableStream is closed'));
    }
    assert(state === 'writable' || state === 'erroring');
...
function writableStreamHasOperationMarkedInFlight(stream) {
    const { inFlightWriteRequest, inFlightCloseRequest, } = stream[kState];
...
function writableStreamDefaultWriterWrite(writer, chunk) {
    ...
    if (stream !== writer[kState].stream) {
        return Promise.reject(new ERR_INVALID_STATE.TypeError('Mismatched WritableStreams'));
    }
    const { state, } = stream[kState];
    if (state === 'errored')
...
function writableStreamDefaultControllerGetDesiredSize(controller) {
    const { highWaterMark, queueTotalSize, } = controller[kState];
    return highWaterMark - queueTotalSize;
}
function writableStreamDefaultControllerGetChunkSize(controller, chunk) {
    try {
        return controller[kState].sizeAlgorithm.call(undefined, chunk);
    }
    catch (error) {
        writableStreamDefaultControllerErrorIfNeeded(controller, error);
        return 1;
...
function setupWritableStreamDefaultControllerFromSink(stream, sink, highWaterMark, sizeAlgorithm) {
    ...
    const startAlgorithm = start ? start.bind(sink, controller) :
        nonOpStart;
