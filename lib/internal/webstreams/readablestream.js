'use strict';
const { AbortError, codes: { ERR_ILLEGAL_CONSTRUCTOR, ERR_INVALID_ARG_VALUE, ERR_INVALID_ARG_TYPE, ERR_INVALID_STATE, ERR_INVALID_THIS, ERR_OUT_OF_RANGE, }, } = require('internal/errors');
const { DOMException, } = internalBinding('messaging');
const { isArrayBufferView, isDataView, } = require('internal/util/types');
const { createDeferredPromise, customInspectSymbol: kInspect, isArrayBufferDetached, kEmptyObject, kEnumerableProperty, SideEffectFreeRegExpPrototypeSymbolReplace, } = require('internal/util');
const { validateAbortSignal, validateBuffer, validateObject, kValidateObjectAllowObjects, kValidateObjectAllowObjectsAndNull, } = require('internal/validators');
const { MessageChannel, } = require('internal/worker/io');
const { kDeserialize, kTransfer, kTransferList, markTransferMode, } = require('internal/worker/js_transferable');
const { queueMicrotask, } = require('internal/process/task_queues');
const { kIsDisturbed, kIsErrored, kIsReadable, kIsClosedPromise, kControllerErrorFunction, } = require('internal/streams/utils');
const { structuredClone } = internalBinding('messaging');
const { ArrayBufferViewGetBuffer, ArrayBufferViewGetByteLength, ArrayBufferViewGetByteOffset, AsyncIterator, cloneAsUint8Array, copyArrayBuffer, createPromiseCallback, customInspect, dequeueValue, enqueueValueWithSize, extractHighWaterMark, extractSizeAlgorithm, lazyTransfer, isViewedArrayBufferDetached, isBrandCheck, resetQueue, setPromiseHandled, transferArrayBuffer, nonOpCancel, nonOpPull, nonOpStart, getIterator, iteratorNext, kType, kState, } = require('internal/webstreams/util');
const { WritableStreamDefaultWriter, isWritableStream, isWritableStreamLocked, isWritableStreamDefaultController, isWritableStreamDefaultWriter, writableStreamAbort, writableStreamCloseQueuedOrInFlight, writableStreamDefaultWriterCloseWithErrorPropagation, writableStreamDefaultWriterRelease, writableStreamDefaultWriterWrite, } = require('internal/webstreams/writablestream');
...
class ReadableStream {
    ...
    cancel(reason = undefined) {
        ...
        if (isReadableStreamLocked(this)) {
            return Promise.reject(new ERR_INVALID_STATE.TypeError('ReadableStream is locked'));
        }
    ...
    values(options = kEmptyObject) {
        ...
        function nextSteps() {
            ...
            if (reader[kState].stream === undefined) {
                return Promise.reject(new ERR_INVALID_STATE.TypeError('The reader is not bound to a ReadableStream'));
            }
        ...
        return Object.setPrototypeOf({
            ...
            next() {
                ...
                state.current = state.current !== undefined ? state.current.then(nextSteps, nextSteps) :
                    nextSteps();
                return state.current;
            },
            return(error) {
                return state.current ? state.current.then(() => returnSteps(error), () => returnSteps(error)) :
                    returnSteps(error);
            },
    ...
    [kTransfer]() {
        ...
        const { writable, promise, } = lazyTransfer().newCrossRealmWritableSink(this, this[kState].transfer.port1);
...
class ReadableStreamBYOBRequest {
    ...
    respond(bytesWritten) {
        ...
        const viewBufferByteLength = viewBuffer.byteLength;
...
class ReadableStreamDefaultReader {
    ...
    read() {
        ...
        if (this[kState].stream === undefined) {
            return Promise.reject(new ERR_INVALID_STATE.TypeError('The reader is not attached to a stream'));
        }
...
class ReadableByteStreamController {
    ...
    get byobRequest() {
        ...
        if (this[kState].byobRequest === null &&
            ...
            const { buffer, byteOffset, bytesFilled, byteLength, } = this[kState].pendingPullIntos[0];
    ...
    enqueue(chunk) {
        ...
        const chunkBufferByteLength = chunkBuffer.byteLength;
...
function readableStreamPipeTo(source, dest, preventClose, preventAbort, preventCancel, signal) {
    ...
    function finalize(rejected, error) {
        ...
        readableStreamReaderGenericRelease(reader);
        if (signal !== undefined)
            disposable?.[Symbol.for("Symbol.dispose")]();
        if (rejected)
    ...
    function shutdown(rejected, error) {
        ...
        if (dest[kState].state === 'writable' &&
            !writableStreamCloseQueuedOrInFlight(dest)) {
            waitForCurrentWrite().then(() => finalize(rejected, error), (error) => finalize(true, error));
            return;
        }
...
function readableByteStreamTee(stream) {
    ...
    function forwardReaderError(thisReader) {
        thisReader[kState].close.promise.then(undefined, (error) => {
...
function readableByteStreamControllerConvertPullIntoDescriptor(desc) {
    const { buffer, bytesFilled, byteLength, byteOffset, ctor, elementSize, } = desc;
...
function readableStreamDefaultControllerEnqueue(controller, chunk) {
    ...
    if (isReadableStreamLocked(stream) &&
        ...
        try {
            const chunkSize = controller[kState].sizeAlgorithm.call(undefined, chunk);
...
function setupReadableStreamDefaultControllerFromSource(stream, source, highWaterMark, sizeAlgorithm) {
    ...
    const startAlgorithm = start ? start.bind(source, controller) :
        nonOpStart;
...
function readableByteStreamControllerClose(controller) {
    const { closeRequested, pendingPullIntos, queueTotalSize, stream, } = controller[kState];
...
function readableByteStreamControllerEnqueue(controller, chunk) {
    const { closeRequested, pendingPullIntos, queue, stream, } = controller[kState];
...
function readableByteStreamControllerEnqueueDetachedPullIntoToQueue(controller, desc) {
    const { buffer, byteOffset, bytesFilled, type, } = desc;
    assert(type === 'none');
    if (bytesFilled > 0) {
...
function readableByteStreamControllerFillPullIntoDescriptorFromQueue(controller, desc) {
    const { buffer, byteLength, byteOffset, bytesFilled, minimumFill, elementSize, } = desc;
    const maxBytesToCopy = Math.min(controller[kState].queueTotalSize, byteLength - bytesFilled);
    ...
    while (totalBytesToCopyRemaining) {
        ...
        const arrayBufferByteLength = buffer.byteLength;
...
function readableByteStreamControllerRespondInReadableState(controller, bytesWritten, desc) {
    const { buffer, bytesFilled, byteLength, type, } = desc;
    ...
    if (remainderSize) {
        ...
        const remainder = buffer.slice(start, end);
...
function readableByteStreamControllerFillReadRequestFromQueue(controller, readRequest) {
    const { queue, queueTotalSize, } = controller[kState];
    assert(queueTotalSize > 0);
    const { buffer, byteOffset, byteLength, } = queue.shift();
    controller[kState].queueTotalSize -= byteLength;
    readableByteStreamControllerHandleQueueDrain(controller);
...
function readableByteStreamControllerProcessReadRequestsUsingQueue(controller) {
    const { stream, queueTotalSize, } = controller[kState];
    const { reader } = stream[kState];
    ...
    while (reader[kState].readRequests.length > 0) {
        ...
        readableByteStreamControllerFillReadRequestFromQueue(controller, reader[kState].readRequests.shift());
    }
}
function readableByteStreamControllerPullSteps(controller, readRequest) {
    const { pendingPullIntos, queueTotalSize, stream, } = controller[kState];
    assert(readableStreamHasDefaultReader(stream));
    if (queueTotalSize) {
