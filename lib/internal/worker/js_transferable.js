'use strict';
const { messaging_deserialize_symbol, messaging_transfer_symbol, messaging_clone_symbol, messaging_transfer_list_symbol, } = internalBinding('symbols');
const { setDeserializerCreateObjectFunction, } = internalBinding('messaging');
const { privateSymbols: { transfer_mode_private_symbol, }, constants: { kDisallowCloneAndTransfer, kTransferable, kCloneable, }, } = internalBinding('util');
function setup() {
    ...
    setDeserializerCreateObjectFunction((deserializeInfo) => {
        const { 0: module, 1: ctor } = deserializeInfo.split(':');
