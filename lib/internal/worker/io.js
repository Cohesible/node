'use strict';
const { kEmptyObject, kEnumerableProperty, setOwnProperty, } = require('internal/util');
const { handle_onclose: handleOnCloseSymbol, oninit: onInitSymbol, no_message_symbol: noMessageSymbol, } = internalBinding('symbols');
const { MessagePort, MessageChannel, broadcastChannel, drainMessagePort, moveMessagePortToContext, receiveMessageOnPort: receiveMessageOnPort_, stopMessagePort, checkMessagePort, DOMException, } = internalBinding('messaging');
const { getEnvMessagePort, } = internalBinding('worker');
const { Readable, Writable } = require('stream');
const { Event, EventTarget, NodeEventTarget, defineEventHandler, initNodeEventTarget, kCreateEvent, kNewListener, kRemoveListener, } = require('internal/event_target');
const { inspect } = require('internal/util/inspect');
const { codes: { ERR_INVALID_ARG_TYPE, ERR_INVALID_THIS, ERR_MISSING_ARGS, }, } = require('internal/errors');
...
const MessagePortPrototype = Object.create(Object.getPrototypeOf(MessagePort.prototype), messagePortPrototypePropertyDescriptors);
...
MessagePort.prototype.hasRef = function () {
    return !!MessagePortPrototype.hasRef.call(this);
};
...
class MessageEvent extends Event {
    constructor(type, { data = null, origin = '', lastEventId = '', source = null, ports = [], } = kEmptyObject) {
        super(type);
        this[kData] = data;
        this[kOrigin] = `${origin}`;
...
Object.defineProperty(MessagePort.prototype, inspect.custom, {
    ...
    value: function inspect() {
        ...
        return Object.assign({ __proto__: MessagePort.prototype }, ref === undefined ? {
            active: false,
        } : {
            active: true,
            refed: ref,
