...
function copyError(source) {
    ...
    const target = { __proto__: Object.getPrototypeOf(source) };
...
function createErrDiff(actual, expected, operator) {
    ...
    for (i = 0; i < maxLines; i++) {
        if (maxLength < i + 1) {
            ...
            //   ]
            //
            if (divergingLines && expectedLine.endsWith(',') &&
...
class AssertionError extends Error {
    constructor(options) {
        ...
        const { message, operator, stackStartFn, details, 
