...
const { ERR_INVALID_ARG_VALUE, ERR_USE_AFTER_CLOSE, } = codes;
const { validateAbortSignal, validateArray, validateNumber, validateString, validateUint32, } = require('internal/validators');
const { kEmptyObject } = require('internal/util');
const { inspect, getStringWidth, stripVTControlCharacters, } = require('internal/util/inspect');
...
function InterfaceConstructor(input, output, completer, terminal) {
    ...
    function onend() {
        if (typeof self[kLine_buffer] === 'string' &&
...
class Interface extends InterfaceConstructor {
    ...
    [kNormalWrite](b) {
        ...
        if (this[kSawReturnAt] &&
            Date.now() - this[kSawReturnAt] <= this.crlfDelay) {
            if (string.codePointAt() === 10)
        ...
        if (newPartContainsEnding !== null) {
            ...
            this[kLine_buffer] = string.slice(indexes[lastIndex]);
            for (let i = 1; i < lastIndex; i += 2) {
                this[kOnLine](string.slice(indexes[i - 1], indexes[i]));
            }
        }
        else if (string) {
    ...
    [kDeleteWordLeft]() {
        if (this.cursor > 0) {
            ...
            this.line =
                leading + this.line.slice(this.cursor, this.line.length);
