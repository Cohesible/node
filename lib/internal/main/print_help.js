...
function format({ options, aliases = new SafeMap(), firstColumn, secondColumn }) {
    ...
    const sortedOptions = [...options.entries()].sort(({ 0: name1, 1: option1 }, { 0: name2, 1: option2 }) => {
        if (option1.defaultIsTrue) {
            name1 = `--no-${name1.slice(2)}`;
        }
        if (option2.defaultIsTrue) {
            name2 = `--no-${name2.slice(2)}`;
        }
        return name1.localeCompare(name2);
    });
    for (const { 0: name, 1: { helpText, type, value, defaultIsTrue }, } of sortedOptions) {
        if (!helpText)
            continue;
        let displayName = name;
        if (defaultIsTrue) {
            displayName = `--no-${displayName.slice(2)}`;
        }
        ...
        text += indent(fold(displayHelpText, secondColumn), firstColumn).trimStart() + '\n';
    }
