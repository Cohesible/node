'use strict';
const { prepareMainThreadExecution, markBootstrapComplete, } = require('internal/process/pre_execution');
const { triggerUncaughtException, exitCodes: { kNoFailure }, } = internalBinding('errors');
...
const kWatchedPaths = getOptionValue('--watch-path').map((path) => resolve(path));
const kPreserveOutput = getOptionValue('--watch-preserve-output');
const kCommand = process.argv.slice(1);
const kCommandStr = inspect(kCommand.join(' '));
const args = process.execArgv.filter((arg, i, arr) => !arg.startsWith('--watch-path') &&
    (!arr[i - 1] || !arr[i - 1].startsWith('--watch-path')) &&
    arg !== '--watch' && !arg.startsWith('--watch=') && arg !== '--watch-preserve-output');
