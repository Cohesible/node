'use strict';
const { BuiltinModule: { normalizeRequirableId, isSynapseBuiltin } } = require('internal/bootstrap/realm');
const { compileSerializeMain, anonymousMainPath, } = internalBinding('mksnapshot');
const { isExperimentalSeaWarningNeeded } = internalBinding('sea');
const { emitExperimentalWarning } = require('internal/util');
const { emitWarningSync } = require('internal/process/warning');
const { initializeCallbacks, namespace: { addSerializeCallback, addDeserializeCallback, isBuildingSnapshot, }, } = require('internal/v8/startup_snapshot');
...
function requireForUserSnapshot(id) {
    let normalizedId = normalizeRequirableId(id);
    if (!normalizedId && !isSynapseBuiltin(id)) {
    ...
    if (isBuildingSnapshot() && !supportedInUserSnapshot(normalizedId)) {
        if (!warnedModules.has(normalizedId)) {
            ...
            // crash.
            emitWarningSync(`It's not yet fully verified whether built-in module "${id}" ` +
...
function main() {
    ...
    addDeserializeCallback(() => {
        if (stackTraceLimitDesc !== undefined) {
            Object.defineProperty(Error, 'stackTraceLimit', stackTraceLimitDesc);
        }
    });
    addSerializeCallback(() => {
        stackTraceLimitDesc = Object.getOwnPropertyDescriptor(Error, 'stackTraceLimit');
        if (stackTraceLimitDesc !== undefined) {
            // We want to use null-prototype objects to not rely on globally mutable
            // %Object.prototype%.
            Object.setPrototypeOf(stackTraceLimitDesc, null);
