...
const { validateArray, validateInt32, validateOneOf, validateString, } = require('internal/validators');
let binding;
...
const IPv6RE = /^\[([^[\]]*)\]/;
const addrSplitRE = /(^.+?)(?::(\d+))?$/;
const { ERR_DNS_SET_SERVERS_FAILED, ERR_INVALID_ARG_VALUE, ERR_INVALID_IP_ADDRESS, } = errors.codes;
const { namespace: { addSerializeCallback, addDeserializeCallback, isBuildingSnapshot, }, } = require('internal/v8/startup_snapshot');
...
class ResolverBase {
    ...
    [kSetServersInteral](newSet, servers) {
        const orig = (this._handle.getServers() || []).map((val) => {
...
function emitInvalidHostnameWarning(hostname) {
    if (!invalidHostnameWarningEmitted) {
        process.emitWarning(`The provided hostname "${hostname}" is not a valid ` +
            'hostname, and is supported in the dns module solely for compatibility.', 'DeprecationWarning', 'DEP0118');
...
function createResolverClass(resolver) {
    const resolveMap = CreateNullProtoObject();
