...
const { kIsNodeError, } = require('internal/errors');
    ...
    const preparedTrace = trace.map((t, i) => {
        ...
        try {
            ...
            if (sm) {
                ...
                // start at 1/1:
                const { originalLine, originalColumn, originalSource, } = sm.findEntry(t.getLineNumber() - 1, t.getColumnNumber() - 1);
...
function getErrorSource(sourceMap, originalSourcePath, originalLine, originalColumn) {
    const originalSourcePathNoScheme = originalSourcePath.startsWith('file://') ?
        fileURLToPath(originalSourcePath) : originalSourcePath;
    const source = getOriginalSource(sourceMap.payload, originalSourcePath);
    ...
    // spaces are used:
    let prefix = '';
    for (const character of line.slice(0, originalColumn + 1)) {
        prefix += character === '\t' ? '\t' : ' '.repeat(getStringWidth(character));
    }
    prefix = prefix.slice(0, -1); // The last character is '^'.
    const exceptionLine = `${originalSourcePathNoScheme}:${originalLine + 1}\n${line}\n${prefix}^\n\n`;
...
function getOriginalSource(payload, originalSourcePath) {
    ...
    const sourceContentIndex = payload.sources.indexOf(originalSourcePath);
    ...
    else if (originalSourcePath.startsWith('file://')) {
...
function getSourceMapErrorSource(fileName, lineNumber, columnNumber) {
    ...
    const { originalLine, originalColumn, originalSource, } = sm.findEntry(lineNumber - 1, columnNumber);
