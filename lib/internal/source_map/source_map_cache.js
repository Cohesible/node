...
function setSourceMapsEnabled(val) {
    ...
    if (val) {
        ...
        const { defaultPrepareStackTrace, } = require('internal/errors');
...
function extractSourceURLMagicComment(content) {
    ...
    while ((match = kSourceURLMagicComment.exec(content))) {
    ...
    if (sourceURL != null && kLeadingProtocol.exec(sourceURL) === null) {
...
function maybeCacheSourceMap(filename, content, cjsModuleInstance, isGeneratedSource, sourceURL, sourceMapURL) {
    ...
    if (!(process.env.NODE_V8_COVERAGE || sourceMapsEnabled))
        return;
...
function findSourceMap(sourceURL) {
    if (kLeadingProtocol.exec(sourceURL) === null) {
        sourceURL = pathToFileURL(sourceURL).href;
    }
    if (!SourceMap) {
