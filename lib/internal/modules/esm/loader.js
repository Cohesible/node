...
const { kIsExecuting, kRequiredModuleSymbol, } = require('internal/modules/cjs/loader');
const { imported_cjs_symbol } = internalBinding('symbols');
const assert = require('internal/assert');
const { ERR_REQUIRE_CYCLE_MODULE, ERR_REQUIRE_ESM, ERR_NETWORK_IMPORT_DISALLOWED, ERR_UNKNOWN_MODULE_FORMAT, } = require('internal/errors').codes;
const { getOptionValue } = require('internal/options');
const { isURL, pathToFileURL, URL } = require('internal/url');
const { emitExperimentalWarning, kEmptyObject } = require('internal/util');
const { compileSourceTextModule, getDefaultConditions, } = require('internal/modules/esm/utils');
...
class ModuleLoader {
    ...
    getModuleWrapForRequire(specifier, parentURL, importAttributes) {
        ...
        if (canParse(specifier)) {
            ...
            if (protocol === 'https:' || protocol === 'http:') {
                throw new ERR_NETWORK_IMPORT_DISALLOWED(specifier, parentURL, 'ES modules cannot be loaded by require() from the network');
            }
        ...
        const wrap = translator.call(this, responseURL, source, isMain);
    ...
    #createModuleJob(url, importAttributes, parentURL, format, sync) {
        const callTranslator = ({ format: finalFormat, responseURL, source }, isMain) => {
            ...
            return translator.call(this, responseURL, source, isMain);
        };
        ...
        const isMain = parentURL === undefined;
        const inspectBrk = (isMain &&
            getOptionValue('--inspect-brk'));
        if (process.env.WATCH_REPORT_DEPENDENCIES && process.send) {
            process.send({ 'watch:import': [url] });
        }
        const { ModuleJob } = require('internal/modules/esm/module_job');
        const job = new ModuleJob(this, url, importAttributes, moduleProvider, isMain, inspectBrk, sync);
        this.loadCache.set(url, importAttributes.type, job);
        return job;
    }
    /**
    ...
    if (!require('internal/modules/esm/utils').forceDefaultLoader()) {
        ...
        if (userLoaderPaths.length > 0) {
            if (!emittedLoaderFlagWarning) {
                ...
                ].reduce((str, { 0: regex, 1: replacement }) => str.replace(regex, replacement), encodeURIComponent(string));
                process.emitWarning('`--experimental-loader` may be removed in the future; instead use `register()`:\n' +
                    `--import 'data:text/javascript,import { register } from "node:module"; import { pathToFileURL } from "node:url"; ${userLoaderPaths.map((loader) => `register(${readableURIEncode(JSON.stringify(loader))}, pathToFileURL("./"))`).join('; ')};'`, 'ExperimentalWarning');
