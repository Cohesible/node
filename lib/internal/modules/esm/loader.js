...
const { ERR_REQUIRE_ESM, ERR_UNKNOWN_MODULE_FORMAT, } = require('internal/errors').codes;
const { getOptionValue } = require('internal/options');
const { pathToFileURL, isURL } = require('internal/url');
const { emitExperimentalWarning } = require('internal/util');
const { getDefaultConditions, } = require('internal/modules/esm/utils');
...
class ModuleLoader {
    ...
    #createModuleJob(url, importAttributes, parentURL, format, sync) {
        ...
        const ModuleJob = require('internal/modules/esm/module_job');
        const job = new ModuleJob(this, url, importAttributes, moduleProvider, parentURL === undefined, inspectBrk, sync);
...
function register(specifier, parentURL = undefined, options) {
    ...
    moduleLoader.register(specifier, parentURL ?? 'data:', options?.data, options?.transferList);
}
module.exports = {
    createModuleLoader,
    getHooksProxy,
