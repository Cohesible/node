...
const { source_text_module_default_hdo, vm_dynamic_import_default_internal, vm_dynamic_import_main_context_default, vm_dynamic_import_missing_flag, vm_dynamic_import_no_callback, vm_dynamic_import_use_filename} = internalBinding('symbols');
const { ModuleWrap } = internalBinding('module_wrap');
const { maybeCacheSourceMap, } = require('internal/source_map/source_map_cache');
const { ERR_VM_DYNAMIC_IMPORT_CALLBACK_MISSING_FLAG, ERR_VM_DYNAMIC_IMPORT_CALLBACK_MISSING, ERR_INVALID_ARG_VALUE, } = require('internal/errors').codes;
const { getOptionValue } = require('internal/options');
const { loadPreloadModules, initializeFrozenIntrinsics, } = require('internal/process/pre_execution');
const { emitExperimentalWarning, getCWDURL, } = require('internal/util');
const { setImportModuleDynamicallyCallback, setInitializeImportMetaObjectCallback, } = internalBinding('module_wrap');
...
/**
 * @type {Map<string, ModuleRegistry>}
 */
const namedModuleRegistries = new Map();
 ...
 */
function registerModule(referrer, registry, name) {
    ...
    if (idSymbol === vm_dynamic_import_use_filename) {
        namedModuleRegistries.set(name, registry);
    } else {
...
async function importModuleDynamicallyCallback(referrerSymbol, specifier, attributes, referrerName) {
    ...
    if (namedModuleRegistries.has(referrerName)) {
        const { importModuleDynamically, callbackReferrer } = namedModuleRegistries.get(referrerName);
        if (importModuleDynamically !== undefined) {
            return importModuleDynamically(specifier, callbackReferrer, attributes);
        }
    }
...
async function initializeHooks() {
    ...
    for (let i = 0; i < customLoaderURLs.length; i++) {
        await hooks.register(customLoaderURLs[i], parentURL);
    }
    return hooks;
}
/**
