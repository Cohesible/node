...
const { source_text_module_default_hdo, vm_dynamic_import_default_internal, vm_dynamic_import_main_context_default, vm_dynamic_import_missing_flag, vm_dynamic_import_no_callback, } = internalBinding('symbols');
...
const moduleRegistries2 = new Map();
...
function registerModule(referrer, registry, name) {
    ...
    if (name) {
        moduleRegistries2.set(name, registry);
    } else {
...
async function importModuleDynamicallyCallback(referrerSymbol, specifier, attributes, referrerName) {
    ...
    if (moduleRegistries2.has(referrerName)) {
        const { importModuleDynamically, callbackReferrer } = moduleRegistries2.get(referrerName);
        if (importModuleDynamically !== undefined) {
            return importModuleDynamically(specifier, callbackReferrer, attributes);
        }
    }
