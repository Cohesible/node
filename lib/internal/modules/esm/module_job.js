...
class ModuleJob {
    ...
    constructor(loader, url, importAttributes = { __proto__: null }, moduleProvider, isMain, inspectBrk, sync = false) {
        this.loader = loader;
        ...
        this.modulePromise = Reflect.apply(moduleProvider, loader, [url, isMain]);
        if (sync) {
        ...
        try {
            ...
            if (!getSourceMapsEnabled() && e.message.includes(' does not provide an export named')) {
                ...
                if (format === 'commonjs') {
                    ...
                    const destructuringAssignment = oneLineNamedImports && oneLineNamedImports.replace(/\s+as\s+/g, ': ');
