...
const { extensionFormatMap, getFormatOfExtensionlessFile, mimeToFormat, } = require('internal/modules/esm/formats');
 ...
 * @returns {string}
 */
function getFileProtocolModuleFormat(url, context = CreateNullProtoObject(), ignoreErrors) {
...
function getHttpProtocolModuleFormat(url, context) {
    if (experimentalNetworkImports) {
        ...
        return Promise.resolve(fetchModule(url, context)).then((entry) => {
...
function defaultGetFormatWithoutErrors(url, context) {
    ...
    if (!Object.hasOwn(protocolHandlers, protocol)) {
        return null;
    }
