...
const { ERR_INPUT_TYPE_NOT_ALLOWED, ERR_INVALID_ARG_TYPE, ERR_INVALID_MODULE_SPECIFIER, ERR_INVALID_PACKAGE_CONFIG, ERR_INVALID_PACKAGE_TARGET, ERR_MANIFEST_DEPENDENCY_MISSING, ERR_MODULE_NOT_FOUND, ERR_PACKAGE_IMPORT_NOT_DEFINED, ERR_PACKAGE_PATH_NOT_EXPORTED, ERR_UNSUPPORTED_DIR_IMPORT, ERR_UNSUPPORTED_RESOLVE_REQUEST, ERR_NETWORK_IMPORT_DISALLOWED, } = require('internal/errors').codes;
...
function emitTrailingSlashPatternDeprecation(match, pjsonUrl, base) {
    ...
    process.emitWarning(`Use of deprecated trailing slash pattern mapping "${match}" in the ` +
        `"exports" field module resolution of the package at ${pjsonPath}${base ? ` imported from ${fileURLToPath(base)}` :
...
function emitInvalidSegmentDeprecation(target, request, match, pjsonUrl, internal, base, isTarget) {
    ...
    process.emitWarning(`Use of deprecated ${double ? 'double slash' :
        'leading or trailing slash matching'} resolving "${target}" for module ` +
        `request "${request}" ${request !== match ? `matched to "${match}" ` : ''}in the "${internal ? 'imports' : 'exports'}" field module resolution of the package at ${pjsonPath}${base ? ` imported from ${fileURLToPath(base)}` : ''}.`, 'DeprecationWarning', 'DEP0166');
}
/**
...
function emitLegacyIndexDeprecation(url, packageJSONUrl, base, main) {
    ...
    if (!main) {
        ...
        process.emitWarning(`Package ${pkgPath} has a "main" field set to "${main}", ` +
            `excluding the full filename and extension to the resolved file at "${path.slice(pkgPath.length)}", imported from ${basePath}.\n Automatic extension resolution of the "main" field is ` +
...
function finalizeResolution(resolved, base, preserveSymlinks) {
    ...
    const stats = internalModuleStat(toNamespacedPath(path.endsWith('/') ? path.slice(-1) : path));
    ...
    if (stats === 1) {
        ...
        throw new ERR_MODULE_NOT_FOUND(path || resolved.pathname, base && fileURLToPath(base), resolved);
    }
    if (!preserveSymlinks) {
        ...
        resolved =
            pathToFileURL(real + (path.endsWith(sep) ? '/' : ''));
...
function resolvePackageTargetString(target, subpath, match, packageJSONUrl, base, pattern, internal, isPathMap, conditions) {
    ...
    if (invalidSegmentRegEx.exec(target.slice(2)) !== null) {
        if (deprecatedInvalidSegmentRegEx.exec(target.slice(2)) === null) {
            if (!isPathMap) {
                const request = pattern ? match.replace('*', () => subpath) :
                    match + subpath;
                const resolvedTarget = pattern ? target.replace(patternRegEx, () => subpath) :
                    target;
...
function packageExportsResolve(packageJSONUrl, packageSubpath, packageConfig, base, conditions) {
    ...
    if (Object.hasOwn(exports, packageSubpath) &&
...
function packageResolve(specifier, base, conditions) {
    ...
    let lastPath;
    do {
        const stat = internalModuleStat(toNamespacedPath(packageJSONPath.slice(0, packageJSONPath.length - 13)));
...
function moduleResolve(specifier, base, conditions, preserveSymlinks) {
    const protocol = typeof base === 'string' ? base.slice(0, base.indexOf(':') + 1) :
        base.protocol;
    const isData = protocol === 'data:';
    const isRemote = isData ||
        protocol === 'http:' ||
        protocol === 'https:';
...
function resolveAsCommonJS(specifier, parentURL) {
    try {
        ...
        if (isRelativeSpecifier(specifier)) {
            ...
            found = relativePosixPath(parentURL.slice('file://'.length, parentURL.lastIndexOf('/')), foundURL);
            ...
            const pkg = i === -1 ? specifier : specifier.slice(0, i);
            const needle = `${sep}node_modules${sep}${pkg}${sep}`;
            const index = found.lastIndexOf(needle);
            if (index !== -1) {
                found = pkg + '/' + found.slice(index + needle.length).split(sep).map(
...
function defaultResolve(specifier, context = {}) {
    ...
    if (parentURL && policy?.manifest) {
        ...
        if (redirects) {
            ...
            const destination = resolve(specifier, new Set(conditions));
            let missing = true;
            if (destination === true) {
                missing = false;
            }
            ...
            if (missing) {
                ...
                reaction(new ERR_MANIFEST_DEPENDENCY_MISSING(parentURL, specifier, [...conditions].join(', ')));
            }
        }
    }
    let parsedParentURL;
    ...
    protocol ??= parsed?.protocol;
    if (protocol === 'node:') {
        return { __proto__: null, url: specifier };
    }
    ...
    let url;
    try {
        url = moduleResolve(specifier, parentURL, conditions, isMain ? preserveSymlinksMain : preserveSymlinks);
    }
    catch (error) {
...
function decorateErrorWithCommonJSHints(error, specifier, parentURL) {
    ...
    if (found && found !== specifier) { // Don't suggest the same input the user provided.
        ...
        const endOfFirstLine = error.stack.indexOf('\n');
        const hint = `Did you mean to import ${JSON.stringify(found)}?`;
        error.stack =
            error.stack.slice(0, endOfFirstLine) + '\n' +
                hint + error.stack.slice(endOfFirstLine);
        error.message += `\n${hint}`;
    }
}
module.exports = {
...
if (policy) {
    ...
    module.exports.defaultResolve = function defaultResolve(specifier, context) {
        const ret = $defaultResolve(specifier, context);
        // This is a preflight check to avoid data exfiltration by query params etc.
        policy.manifest.mightAllow(ret.url, () => new ERR_MANIFEST_DEPENDENCY_MISSING(context.parentURL, specifier, context.conditions));
        return ret;
    };
}
