...
const { ERR_INTERNAL_ASSERTION, ERR_INVALID_ARG_TYPE, ERR_INVALID_ARG_VALUE, ERR_INVALID_RETURN_PROPERTY_VALUE, ERR_INVALID_RETURN_VALUE, ERR_LOADER_CHAIN_INCOMPLETE, ERR_METHOD_NOT_IMPLEMENTED, ERR_WORKER_UNSERIALIZABLE_ERROR, } = require('internal/errors').codes;
...
const { defaultResolve, throwIfInvalidParentURL, } = require('internal/modules/esm/resolve');
const { getDefaultConditions, loaderWorkerId, } = require('internal/modules/esm/utils');
const { deserializeError } = require('internal/error_serdes');
const { SHARED_MEMORY_BYTE_LENGTH, WORKER_TO_MAIN_THREAD_NOTIFICATION, } = require('internal/modules/esm/shared_constants');
...
class Hooks {
    ...
    async register(urlOrSpecifier, parentURL, data) {
        ...
        const keyedExports = await cascadedLoader.import(urlOrSpecifier, parentURL, kEmptyObject);
    ...
    addCustomLoader(url, exports, data) {
        ...
        if (resolve) {
            ...
            this.#chains.resolve.push({ fn: resolve, url, next });
        }
        if (load) {
            const next = this.#chains.load[this.#chains.load.length - 1];
            this.#chains.load.push({ fn: load, url, next });
        }
     ...
     */
    async resolve(originalSpecifier, parentURL, importAttributes = CreateNullProtoObject()) {
        ...
        if (resolvedImportAttributes != null &&
...
class HooksProxy {
    ...
    waitForWorker() {
        if (!this.#isReady) {
            ...
            if (!this.#worker[kIsOnline]) {
                ...
                if (response == null || response.message.status === 'exit') {
                    return;
                }
    ...
    async makeAsyncRequest(method, transferList, ...args) {
        ...
        if (transferList) {
            finalTransferList.push(...transferList);
        }
 ...
 */
function pluckHooks({ globalPreload, initialize, resolve, load, }) {
    const acceptedHooks = CreateNullProtoObject();
    if (resolve) {
...
function nextHookFactory(current, meta, { validateArgs, validateOutput }) {
    ...
    const { hookName } = meta;
    const { fn: hook, url: hookFilePath, next, } = current;
    // ex 'nextResolve'
    const nextHookName = `next${hookName[0].toUpperCase() + hookName.slice(1)}`;
    let nextNextHook;
    if (next) {
    ...
    return Object.defineProperty(async (arg0 = undefined, context) => {
