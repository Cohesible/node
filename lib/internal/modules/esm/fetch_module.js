'use strict';
const { Buffer: { concat: BufferConcat }, } = require('buffer');
const { ERR_NETWORK_IMPORT_DISALLOWED, ERR_NETWORK_IMPORT_BAD_RESPONSE, ERR_MODULE_NOT_FOUND, } = require('internal/errors').codes;
...
function fetchWithRedirects(parsed, context) {
    ...
    const result = (async () => {
        ...
        try {
            const hasLocation = Object.prototype.hasOwnProperty.call(res.headers, 'location');
            if (isRedirect(res.statusCode) && hasLocation) {
                const location = new URL(res.headers.location, parsed);
                if (location.protocol !== 'http:' && location.protocol !== 'https:') {
                    throw new ERR_NETWORK_IMPORT_DISALLOWED(res.headers.location, parsed.href, 'cannot redirect to non-network location');
                }
            ...
            if (!contentType) {
                throw new ERR_NETWORK_IMPORT_BAD_RESPONSE(parsed.href, "the 'Content-Type' header is required");
            }
            /**
             * @type {CacheEntry}
             */
...
function fetchModule(parsed, context) {
    ...
    if (parsed.protocol === 'http:') {
        return isLocalAddress(parsed.hostname).then((is) => {
            if (is !== true) {
                throw new ERR_NETWORK_IMPORT_DISALLOWED(href, parentURL, 'http can only be used to load local resources (use https instead).');
            }
