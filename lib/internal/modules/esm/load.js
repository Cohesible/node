...
const { ERR_INVALID_URL, ERR_UNKNOWN_MODULE_FORMAT, ERR_UNSUPPORTED_ESM_URL_SCHEME, } = require('internal/errors').codes;
...
async function getSource(url, context) {
    ...
    if (protocol === 'file:') {
        ...
        const match = DATA_URL_PATTERN.exec(url.pathname);
        if (!match) {
    ...
    else if (experimentalNetworkImports && (protocol === 'https:' ||
        protocol === 'http:')) {
        ...
        if (experimentalNetworkImports) {
            supportedSchemes.push('http', 'https');
        }
