...
const { loadBuiltinModule, stripBOM, } = require('internal/modules/helpers');
const { cjsExportsCache, importedCJSCache, kIsCachedByESMLoader, Module: CJSModule, } = require('internal/modules/cjs/loader');
const { fileURLToPath, pathToFileURL, URL } = require('internal/url');
let debug = require('internal/util/debuglog').debuglog('esm', (fn) => {
    debug = fn;
});
const { emitExperimentalWarning, kEmptyObject, setOwnProperty } = require('internal/util');
const { ERR_UNKNOWN_BUILTIN_MODULE, ERR_INVALID_RETURN_PROPERTY_VALUE, } = require('internal/errors').codes;
...
function enrichCJSError(err, content, filename) {
    if (err != null && Object.getPrototypeOf(err) === SyntaxError.prototype &&
        containsModuleSyntax(content, filename)) {
        // Emit the warning synchronously because we are in the middle of handling
        // a SyntaxError that will throw and likely terminate the process before an
        // asynchronous warning would be emitted.
        emitWarningSync('To load an ES module, set "type": "module" in the package.json or use ' +
            'the .mjs extension.');
    }
}
/**
...
function loadCJSModule(module, source, url, filename) {
    ...
    const requireFn = function require(specifier) {
        ...
        if (!specifier.startsWith('node:') && !BuiltinModule.normalizeRequirableId(specifier)) {
    ...
    Reflect.apply(compiledWrapper, module.exports, [module.exports, requireFn, module, filename, __dirname]);
...
function createCJSModuleWrap(url, source, isMain, loadCJS = loadCJSModule) {
    ...
    return new ModuleWrap(url, undefined, namesWithDefault, function () {
        ...
        for (const exportName of exportNames) {
            if (!Object.hasOwn(exports, exportName) ||
                exportName === 'default') {
                continue;
            }
...
translators.set('json', function jsonStrategy(url, source) {
    ...
    const pathname = url.startsWith('file:') ?
    ...
    pathname && !url.includes('?') && !url.includes('#');
    let modulePath;
    let module;
    if (shouldCheckAndPopulateCJSModuleCache) {
        modulePath = isWindows ? pathname.replaceAll('/', '\\') : pathname;
...
translators.set('wasm', async function (url, source) {
    ...
    const imports = WebAssembly.Module.imports(compiled).map(({ module }) => module);
