...
const { hasUncaughtExceptionCaptureCallback, } = require('internal/process/execution');
const { isArrayBuffer, isDataView, isTypedArray, } = require('util/types');
const { receiveMessageOnPort } = require('internal/worker/io');
const { WORKER_TO_MAIN_THREAD_NOTIFICATION, } = require('internal/modules/esm/shared_constants');
...
async function customizedModuleWorker(lock, syncCommPort, errorHandler) {
    ...
    function checkForMessages() {
        ...
        if (response !== undefined) {
            handleMessage(response.message).then(undefined, errorHandler);
        }
    }
...
module.exports = function setupModuleWorker(workerData, syncCommPort) {
    ...
    return customizedModuleWorker(lock, syncCommPort, errorHandler).then(undefined, errorHandler);
};
