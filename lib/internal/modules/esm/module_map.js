...
class ResolveCache extends Map {
    ...
    serializeKey(specifier, importAttributes) {
        ...
        return specifier + '::' + keys.sort().map((key) => JSON.stringify(key) + JSON.stringify(importAttributes[key])).join(',');
    }
    #getModuleCachedImports(parentURL) {
        let internalCache = super.get(parentURL);
        if (internalCache == null) {
            super.set(parentURL, internalCache = CreateNullProtoObject());
        }
...
class LoadCache extends Map {
    ...
    set(url, type = kImplicitAssertType, job) {
        ...
        debug(`Storing ${url} (${type === kImplicitAssertType ? 'implicit type' : type}) in ModuleLoadMap`);
        const cachedJobsForUrl = super.get(url) ?? CreateNullProtoObject();
