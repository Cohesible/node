'use strict';
const { ERR_INVALID_ARG_TYPE, ERR_MANIFEST_DEPENDENCY_MISSING, ERR_UNKNOWN_BUILTIN_MODULE, } = require('internal/errors').codes;
const { BuiltinModule } = require('internal/bootstrap/realm');
const { shouldRetryAsESM: contextifyShouldRetryAsESM, constants: { syntaxDetectionErrors: { esmSyntaxErrorMessages, throwsOnlyInCommonJSErrorMessages, }, }, } = internalBinding('contextify');
...
function makeRequireFunction(mod, redirects) {
    ...
    if (redirects) {
        ...
        require = function require(specifier) {
            ...
            if (missing) {
                reaction(new ERR_MANIFEST_DEPENDENCY_MISSING(id, specifier, [...conditions].join(', ')));
            }
...
function normalizeReferrerURL(referrerName) {
    ...
    if (typeof referrerName === 'string') {
        ...
        if (referrerName.startsWith('file://') ||
...
function shouldRetryAsESM(errorMessage, source) {
    esmSyntaxErrorMessagesSet ??= new Set(esmSyntaxErrorMessages);
    if (esmSyntaxErrorMessagesSet.has(errorMessage)) {
        return true;
    }
    throwsOnlyInCommonJSErrorMessagesSet ??= new Set(throwsOnlyInCommonJSErrorMessages);
