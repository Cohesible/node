...
function deserializePackageJSON(path, contents, checkIntegrity = false) {
    ...
    let pjsonPath = path;
    const { 0: name, 1: main, 2: type, 3: plainImports, 4: plainExports, 5: manifest, 6: optionalFilePath, } = contents;
...
function read(jsonPath, { base, specifier, isESM } = kEmptyObject) {
    ...
    const parsed = modulesBinding.readPackageJSON(jsonPath, isESM, base == null ? undefined : `${base}`, specifier == null ? undefined : `${specifier}`);
...
function getNearestParentPackageJSON(checkPath) {
    ...
    const path = data.pjsonPath.slice(0, data.pjsonPath.lastIndexOf(sep));
    return { data, path };
}
/**
