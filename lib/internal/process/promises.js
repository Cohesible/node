'use strict';
const { tickInfo, promiseRejectEvents: { kPromiseRejectWithNoHandler, kPromiseHandlerAddedAfterReject, kPromiseResolveAfterResolved, kPromiseRejectAfterResolved, }, setPromiseRejectCallback, } = internalBinding('task_queue');
const { deprecate } = require('internal/util');
const { noSideEffectsToString, triggerUncaughtException, exitCodes: { kGenericUserError }, } = internalBinding('errors');
const { pushAsyncContext, popAsyncContext, symbols: { async_id_symbol: kAsyncIdSymbol, trigger_async_id_symbol: kTriggerAsyncIdSymbol, }, } = require('internal/async_hooks');
...
function isErrorLike(o) {
    return typeof o === 'object' &&
        o !== null && Object.prototype.hasOwnProperty.call(o, 'stack');
}
...
const multipleResolvesDeprecate = deprecate(() => { }, 'The multipleResolves event has been deprecated.', 'DEP0160');
...
function emitUnhandledRejectionWarning(uid, reason) {
    const warning = getErrorWithoutStack(unhandledRejectionErrName, 'Unhandled promise rejection. This error originated either by ' +
...
function processPromiseRejections() {
    ...
    while (len--) {
        ...
        let needPop = true;
        const { [kAsyncIdSymbol]: promiseAsyncId, [kTriggerAsyncIdSymbol]: promiseTriggerAsyncId, } = promise;
        ...
        if (typeof promiseAsyncId !== 'undefined') {
            pushAsyncContext(promiseAsyncId, promiseTriggerAsyncId, promise);
        }
        try {
...
function generateUnhandledRejectionError(reason) {
    const message = 'This error originated either by ' +
        'throwing inside of an async function without a catch block, ' +
        'or by rejecting a promise which was not handled with .catch().' +
