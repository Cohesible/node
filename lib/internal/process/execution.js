...
const { codes: { ERR_INVALID_ARG_TYPE, ERR_UNCAUGHT_EXCEPTION_CAPTURE_ALREADY_SET, ERR_EVAL_ESM_CANNOT_PRINT, }, } = require('internal/errors');
const { pathToFileURL } = require('internal/url');
const { exitCodes: { kGenericUserError } } = internalBinding('errors');
const { executionAsyncId, clearDefaultTriggerAsyncId, clearAsyncIdStack, hasAsyncIdStack, afterHooksExist, emitAfter, popAsyncContext, } = require('internal/async_hooks');
...
function evalModuleEntryPoint(source, print) {
    ...
    /^/.exec(''); // Necessary to reset RegExp statics before user code runs.
    return require('internal/modules/run_main').runEntryPointWithESMLoader((loader) => loader.eval(source, getEvalModuleUrl(), true));
}
function evalScript(name, body, breakFirstLine, print, shouldLoadESM = false) {
    ...
    `;
        ...
        const result = module._compile(script, `${name}-wrapper`)(() => {
            ...
            const script = makeContextifyScript(body, // code
            name, // filename,
            0, // lineOffset
            0, // columnOffset,
