'use strict';
const { getOptionValue, refreshOptions, getEmbedderOptions, } = require('internal/options');
const { reconnectZeroFillToggle } = require('internal/buffer');
const { exposeInterface, exposeLazyInterfaces, defineReplaceableLazyAttribute, setupCoverageHooks, } = require('internal/util');
const { ERR_INVALID_THIS, ERR_MANIFEST_ASSERT_INTEGRITY, ERR_NO_CRYPTO, ERR_MISSING_OPTION, ERR_ACCESS_DENIED, } = require('internal/errors').codes;
const assert = require('internal/assert');
const { namespace: { addSerializeCallback, isBuildingSnapshot, }, } = require('internal/v8/startup_snapshot');
...
function prepareShadowRealmExecution() {
    ...
    const { privateSymbols: { host_defined_option_symbol, }, } = internalBinding('util');
...
function prepareExecution(options) {
    ...
    refreshRuntimeOptions(); // 0.03ms
    reconnectZeroFillToggle(); // 0.02ms
    // Patch the process object and get the resolved main entry point.
    const mainEntry = patchProcessObject(expandArgv1); // 0.55ms
    setupTraceCategoryState(); // 0.02ms
    setupInspectorHooks(); // 0.01ms
    setupNavigator(); // 0.1ms
    setupWarningHandler(); //0.04ms
    setupUndici(); // 0.02ms
    setupWebCrypto(); // 0.05ms
    setupCustomEvent(); // 0.11ms
    setupCodeCoverage(); // 0.01ms
    setupDebugEnv(); // 0.05
    // Process initial diagnostic reporting configuration, if present.
    initializeReport(); // 0.01
    // Load permission system API
    initializePermission(); // 0.05
    initializeSourceMapsHandlers(); // 0.1
    initializeDeprecations(); // 0.05
    require('internal/dns/utils').initializeDns();
    setupSymbolDisposePolyfill();
    if (isMainThread) {
        assert(internalBinding('worker').isMainThread);
        // Worker threads will get the manifest in the message handler.
        const policy = readPolicyFromDisk(); // 0.07
        if (policy) {
            require('internal/process/policy')
                .setup(policy.manifestSrc, policy.manifestURL);
        }
        // Print stack trace on `SIGINT` if option `--trace-sigint` presents.
        setupStacktracePrinterOnSigint(); // 0.02
        initializeReportSignalHandlers(); // 0.01 Main-thread-only.
        initializeHeapSnapshotSignalHandlers(); // 0.02
        // If the process is spawned with env NODE_CHANNEL_FD, it's probably
        // spawned by our child_process module, then initialize IPC.
        // This attaches some internal event listeners and creates:
        // process.send(), process.channel, process.connected,
        // process.disconnect().
        setupChildProcessIpcChannel(); // 0.12
        // If this is a worker in cluster mode, start up the communication
        // channel. This needs to be done before any user code gets executed
        // (including preload modules).
        initializeClusterIPC(); // 0.02
        // TODO(joyeecheung): do this for worker threads as well.
        require('internal/v8/startup_snapshot').runDeserializeCallbacks(); // 0.02
    }
    else {
        assert(!internalBinding('worker').isMainThread);
...
function setupUserModules(forceDefaultLoader = false) {
    ...
    const { hasStartedUserCJSExecution, hasStartedUserESMExecution, } = require('internal/modules/helpers');
...
function setupWarningHandler() {
    const { onWarning, resetForSerialization, } = require('internal/process/warning');
    ...
    if (internalBinding('config').hasOpenSSL) {
        defineReplaceableLazyAttribute(globalThis, 'internal/crypto/webcrypto', ['crypto'], false, function cryptoThisCheck() {
            if (this !== globalThis && this != null)
                throw new ERR_INVALID_THIS('nullish or must be the global object');
        });
        exposeLazyInterfaces(globalThis, 'internal/crypto/webcrypto', ['Crypto', 'CryptoKey', 'SubtleCrypto']);
    }
    else {
        Object.defineProperty(globalThis, 'crypto', { __proto__: null, ...Object.getOwnPropertyDescriptor({
                get crypto() {
...
function initializeDeprecations() {
    ...
    for (const name of [
        ...
        utilBinding[name] = pendingDeprecation ?
            deprecate(types[name], 'Accessing native typechecking bindings of Node ' +
                'directly is deprecated. ' +
                `Please use \`util.types.${name}\` instead.`, 'DEP0103') :
            types[name];
    }
    ...
    if (pendingDeprecation) {
        process.binding = deprecate(process.binding, 'process.binding() is deprecated. ' +
    ...
    if (experimentalPermission) {
        ...
        for (const flag of warnFlags) {
            if (getOptionValue(flag)) {
                process.emitWarning(`The flag ${flag} must be used with extreme caution. ` +
        ...
        for (const flag of warnCommaFlags) {
            ...
            if (value.length === 1 && value[0].includes(',')) {
                process.emitWarning(`The ${flag} CLI flag has changed. ` +
                    'Passing a comma-separated list of paths is no longer valid. ' +
                    'Documentation can be found at ' +
                    'https://nodejs.org/api/permissions.html#file-system-permissions', 'Warning');
            }
        }
        ...
        availablePermissionFlags.forEach((flag) => {
            const value = getOptionValue(flag);
            if (value.length) {
...
function initializeSourceMapsHandlers() {
    if (getOptionValue('--enable-source-maps')) {
        require('internal/source_map/source_map_cache').setSourceMapsEnabled(true);
    }
}
...
function loadPreloadModules() {
    ...
    if (preloadModules && preloadModules.length > 0) {
        const { Module: { _preloadModules, }, } = require('internal/modules/cjs/loader');
