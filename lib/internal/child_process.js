'use strict';
const { ErrnoException, codes: { ERR_INVALID_ARG_TYPE, ERR_INVALID_ARG_VALUE, ERR_INVALID_HANDLE_TYPE, ERR_INVALID_SYNC_FORK_INPUT, ERR_IPC_CHANNEL_CLOSED, ERR_IPC_DISCONNECTED, ERR_IPC_ONE_PIPE, ERR_IPC_SYNC_FORK, ERR_MISSING_ARGS, }, } = require('internal/errors');
const { validateArray, validateObject, validateOneOf, validateString, } = require('internal/validators');
...
const { WriteWrap, kReadBytesOrError, kArrayBufferOffset, kLastWriteWasAsync, streamBaseState, } = internalBinding('stream_wrap');
...
const { UV_EACCES, UV_EAGAIN, UV_EINVAL, UV_EMFILE, UV_ENFILE, UV_ENOENT, UV_ENOSYS, UV_ESRCH, } = internalBinding('uv');
...
ChildProcess.prototype[Symbol.for("Symbol.dispose")] = function () {
    if (!this.killed) {
        this.kill();
    }
};
...
function setupChannel(target, channel, serializationMode) {
    ...
    target._send = function (message, handle, options, callback) {
        ...
        if (handle) {
            ...
            handle = Reflect.apply(handleConversion[message.type].send, target, [message, handle, options]);
...
function isInternal(message) {
    return (message !== null &&
        ...
        message.cmd.slice(0, INTERNAL_PREFIX.length) ===
            INTERNAL_PREFIX);
}
