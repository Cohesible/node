'use strict';
const { defineEventHandler, EventTarget, Event, kTrustEvent, kNewListener, kRemoveListener, kResistStopPropagation, kWeakHandler, } = require('internal/event_target');
const { createDeferredPromise, customInspectSymbol, kEmptyObject, kEnumerableProperty, } = require('internal/util');
const { inspect } = require('internal/util/inspect');
const { codes: { ERR_ILLEGAL_CONSTRUCTOR, ERR_INVALID_ARG_TYPE, ERR_INVALID_THIS, }, } = require('internal/errors');
const { validateAbortSignal, validateAbortSignalArray, validateObject, validateUint32, kValidateObjectAllowArray, kValidateObjectAllowFunction, } = require('internal/validators');
const { DOMException, } = internalBinding('messaging');
const { clearTimeout, setTimeout, } = require('timers');
const assert = require('internal/assert');
const { kDeserialize, kTransfer, kTransferList, } = require('internal/worker/js_transferable');
let _MessageChannel;
...
class AbortSignal extends EventTarget {
    ...
    constructor(dontThrowSymbol = undefined, init = kEmptyObject) {
        ...
        super();
        const { aborted = false, reason = undefined, transferable = false, composite = false, } = init;
    ...
    static timeout(delay) {
        ...
        clearTimeoutRegistry.register(signal, setWeakAbortSignalTimeout(new WeakRef(signal), delay));
        return signal;
    }
    /**
...
async function aborted(signal, resource) {
    ...
    const opts = { [kWeakHandler]: resource, once: true, [kResistStopPropagation]: true };
