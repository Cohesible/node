...
const { WriteWrap, kReadBytesOrError, kArrayBufferOffset, kBytesWritten, kLastWriteWasAsync, streamBaseState, } = internalBinding('stream_wrap');
const { UV_EOF } = internalBinding('uv');
const { ErrnoException, } = require('internal/errors');
const { owner_symbol } = require('internal/async_hooks').symbols;
const { kTimeout, setUnrefTimeout, getTimerDuration, } = require('internal/timers');
...
function setStreamTimeout(msecs, callback) {
    ...
    if (msecs === 0) {
        ...
        if (this[kBoundSession])
            this[kBoundSession][kUpdateTimer]();
        if (callback !== undefined) {
            validateFunction(callback, 'callback');
