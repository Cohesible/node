'use strict';
const { createBlob: _createBlob, createBlobFromFilePath: _createBlobFromFilePath, concat, getDataObject, } = internalBinding('blob');
...
const { createDeferredPromise, customInspectSymbol: kInspect, kEmptyObject, kEnumerableProperty, lazyDOMException, } = require('internal/util');
const { inspect } = require('internal/util/inspect');
const { codes: { ERR_INVALID_ARG_TYPE, ERR_INVALID_ARG_VALUE, ERR_INVALID_THIS, ERR_INVALID_STATE, ERR_BUFFER_TOO_LARGE, }, } = require('internal/errors');
const { validateDictionary, } = require('internal/validators');
...
class Blob {
    ...
    constructor(sources = [], options) {
        ...
        this[kLength] = length;
        type = `${type}`;
        this[kType] = disallowedTypeCharacters.exec(type) !== null ?
            '' : type.toLowerCase();
    }
    ...
    slice(start = 0, end = this[kLength], contentType = '') {
        ...
        if (disallowedTypeCharacters.exec(contentType) !== null) {
            contentType = '';
        }
        else {
...
function resolveObjectURL(url) {
    ...
    try {
        ...
        if (split.length !== 2)
            return;
        const { 0: base, 1: id, } = split;
        if (base !== 'nodedata')
            return;
        const ret = getDataObject(id);
        if (ret === undefined)
            return;
        const { 0: handle, 1: length, 2: type, } = ret;
        if (handle !== undefined)
