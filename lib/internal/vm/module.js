...
const { customInspectSymbol, getConstructorOf, kEmptyObject, } = require('internal/util');
const { ERR_INVALID_ARG_TYPE, ERR_INVALID_ARG_VALUE, ERR_VM_MODULE_ALREADY_LINKED, ERR_VM_MODULE_DIFFERENT_CONTEXT, ERR_VM_MODULE_CANNOT_CREATE_CACHED_DATA, ERR_VM_MODULE_LINK_FAILURE, ERR_VM_MODULE_NOT_MODULE, ERR_VM_MODULE_STATUS, } = require('internal/errors').codes;
const { validateBoolean, validateBuffer, validateFunction, validateInt32, validateObject, validateUint32, validateString, validateInternalField, } = require('internal/validators');
const binding = internalBinding('module_wrap');
const { ModuleWrap, kUninstantiated, kInstantiating, kInstantiated, kEvaluating, kEvaluated, kErrored, } = binding;
const STATUS_MAP = {
...
function isModule(object) {
    if (typeof object !== 'object' || object === null || !Object.hasOwn(object, kWrap)) {
        return false;
    }
    return true;
}
class Module {
    constructor(options) {
        ...
        let registry = CreateNullProtoObject();
        if (sourceText !== undefined) {
            ...
            registerModule(this[kWrap], registry, options.identifier);
        }
        else {
...
class SourceTextModule extends Module {
    ...
    constructor(sourceText, options = kEmptyObject) {
        ...
        validateObject(options, 'options');
        const { lineOffset = 0, columnOffset = 0, initializeImportMeta, importModuleDynamically, context, identifier, cachedData, } = options;
    ...
    get dependencySpecifiers() {
        ...
        this[kDependencySpecifiers] ??= Object.freeze(this[kWrap].getStaticDependencySpecifiers());
...
class SyntheticModule extends Module {
    constructor(exportNames, evaluateCallback, options = kEmptyObject) {
        if (!Array.isArray(exportNames) || exportNames.some((e) => typeof e !== 'string')) {
