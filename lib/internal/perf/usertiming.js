...
const { validateNumber, validateObject, validateString, validateInternalField, } = require('internal/validators');
const { codes: { ERR_ILLEGAL_CONSTRUCTOR, ERR_INVALID_ARG_VALUE, ERR_MISSING_ARGS, ERR_PERFORMANCE_INVALID_TIMESTAMP, ERR_PERFORMANCE_MEASURE_INVALID_OPTIONS, }, } = require('internal/errors');
const { structuredClone } = internalBinding('messaging');
const { lazyDOMException, kEnumerableProperty, } = require('internal/util');
...
function calculateStartDuration(startOrMeasureOptions, endMark) {
    ...
    if (optionsValid) {
        ...
        if (start !== undefined && end !== undefined && duration !== undefined) {
            throw new ERR_PERFORMANCE_MEASURE_INVALID_OPTIONS('Must not have options.start, options.end, and ' +
