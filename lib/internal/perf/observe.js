'use strict';
const { constants: { NODE_PERFORMANCE_ENTRY_TYPE_GC, NODE_PERFORMANCE_ENTRY_TYPE_HTTP2, NODE_PERFORMANCE_ENTRY_TYPE_HTTP, NODE_PERFORMANCE_ENTRY_TYPE_NET, NODE_PERFORMANCE_ENTRY_TYPE_DNS, }, installGarbageCollectionTracking, observerCounts, removeGarbageCollectionTracking, setupObservers, } = internalBinding('performance');
const { isPerformanceEntry, createPerformanceNodeEntry, } = require('internal/perf/performance_entry');
const { codes: { ERR_ILLEGAL_CONSTRUCTOR, ERR_INVALID_ARG_VALUE, ERR_INVALID_ARG_TYPE, ERR_MISSING_ARGS, }, } = require('internal/errors');
const { validateFunction, validateObject, validateInternalField, } = require('internal/validators');
const { customInspectSymbol: kInspect, deprecate, lazyDOMException, kEmptyObject, kEnumerableProperty, } = require('internal/util');
...
const kDeprecationMessage = 'Custom PerformanceEntry accessors are deprecated. ' +
...
class PerformanceObserverEntryList {
    ...
    getEntriesByType(type) {
        ...
        type = `${type}`;
        return this[kBuffer].filter((entry) => entry.entryType === type);
    }
    getEntriesByName(name, type = undefined) {
        ...
        if (type != null /** not nullish */) {
            return this[kBuffer].filter((entry) => entry.name === name && entry.entryType === type);
        }
        return this[kBuffer].filter((entry) => entry.name === name);
    }
...
class PerformanceObserver {
    ...
    observe(options = kEmptyObject) {
        ...
        if (entryTypes != null && type != null)
            throw new ERR_INVALID_ARG_VALUE('options.entryTypes', entryTypes, 'options.entryTypes can not set with ' +
                'options.type together');
        switch (this.#type) {
            case undefined:
                if (entryTypes !== undefined)
                    this.#type = kTypeMultiple;
                if (type !== undefined)
                    this.#type = kTypeSingle;
                break;
            case kTypeSingle:
                if (entryTypes !== undefined)
    ...
    if (!resourceTimingBufferFullPending) {
        ...
        setImmediate(() => {
            while (resourceTimingSecondaryBuffer.length > 0) {
                ...
                const numbersToPreserve = Math.max(Math.min(resourceTimingBufferSizeLimit - resourceTimingBuffer.length, resourceTimingSecondaryBuffer.length), 0);
...
function clearEntriesFromBuffer(type, name) {
    ...
    if (type === 'mark') {
        markEntryBuffer = name === undefined ?
            [] : markEntryBuffer.filter((entry) => entry.name !== name);
    }
    else if (type === 'measure') {
        measureEntryBuffer = name === undefined ?
            [] : measureEntryBuffer.filter((entry) => entry.name !== name);
    }
    else {
        resourceTimingBuffer = name === undefined ?
            [] : resourceTimingBuffer.filter((entry) => entry.name !== name);
    }
}
function filterBufferMapByNameAndType(name, type) {
    ...
    if (type === 'mark') {
        ...
        bufferList = [];
        bufferList.push(...markEntryBuffer);
        bufferList.push(...measureEntryBuffer);
        bufferList.push(...resourceTimingBuffer);
    }
    if (name !== undefined) {
        bufferList = bufferList.filter((buffer) => buffer.name === name);
    }
    else if (type !== undefined) {
        bufferList = bufferList.slice();
    }
    return bufferList.sort(performanceObserverSorter);
}
function observerCallback(name, type, startTime, duration, details) {
    const entry = createPerformanceNodeEntry(name, type, startTime, duration, details);
...
function stopPerf(target, key, context = {}) {
    ...
    const entry = createPerformanceNodeEntry(ctx.name, ctx.type, startTime, now() - startTime, { ...ctx.detail, ...context.detail });
    enqueue(entry);
}
module.exports = {
    PerformanceObserver,
