...
const { ERR_ILLEGAL_CONSTRUCTOR, } = require('internal/errors').codes;
const { kEnumerableProperty, } = require('internal/util');
const { getAvailableParallelism, } = internalBinding('os');
...
function getNavigatorPlatform(process) {
    ...
    return `${process.platform[0].toUpperCase()}${process.platform.slice(1)} ${process.arch}`;
}
class Navigator {
    ...
    #languages = Object.freeze([this.#language]);
    constructor() {
