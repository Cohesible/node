'use strict';
const { codes: { ERR_INVALID_ARG_TYPE, ERR_EVENT_RECURSION, ERR_MISSING_ARGS, ERR_INVALID_THIS, }, } = require('internal/errors');
...
const { customInspectSymbol, kEmptyObject, kEnumerableProperty, } = require('internal/util');
...
Object.defineProperties(Event, staticProps.reduce((result, staticProp, index = 0) => {
    result[staticProp] = {
        __proto__: null,
        writable: false,
        configurable: false,
...
function weakListeners() {
    weakListenersState ??= new FinalizationRegistry(({ eventTarget, listener, eventType }) => eventTarget.deref()?.[kRemoveWeakListenerHelper](eventType, listener));
...
// slower.
class Listener {
    constructor(eventTarget, eventType, previous, listener, once, capture, passive, isNodeStyleListener, weak, resistStopPropagation) {
        this.next = undefined;
...
class EventTarget {
    ...
    addEventListener(type, listener, options = kEmptyObject) {
        ...
        const { once, capture, passive, signal, isNodeStyleListener, weak, resistStopPropagation, } = validateEventListenerOptions(options);
        ...
        new Listener(this, type, previous, listener, once, capture, passive, isNodeStyleListener, weak, resistStopPropagation);
        root.size++;
