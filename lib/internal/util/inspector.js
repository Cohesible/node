...
function isUsingInspector(execArgv = process.execArgv) {
    if (!_isUsingInspector.has(execArgv)) {
        _isUsingInspector.set(execArgv, execArgv.some((arg) => kInspectArgRegex.exec(arg) !== null) ||
            kInspectArgRegex.exec(process.env.NODE_OPTIONS) !== null);
    }
...
function installConsoleExtensions(commandLineApi) {
    ...
    consoleAPIModule.paths.push(...CJSModule._nodeModulePaths(cwd));
    consoleAPIModule.paths.push(...CJSModule.globalPaths);
...
function wrapConsole(consoleFromNode) {
    ...
    for (const key of Object.keys(consoleFromVM)) {
        ...
        // the original stack.
        if (Object.hasOwn(consoleFromNode, key)) {
            consoleFromNode[key] = consoleCall.bind(consoleFromNode, consoleFromVM[key], consoleFromNode[key]);
