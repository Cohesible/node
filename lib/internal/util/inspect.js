...
const { constants: { ALL_PROPERTIES, ONLY_ENUMERABLE, kPending, kRejected, }, getOwnNonIndexProperties, getPromiseDetails, getProxyDetails, previewEntries, getConstructorName: internalGetConstructorName, getExternalValue, } = internalBinding('util');
const { customInspectSymbol, isError, join, removeColors, } = require('internal/util');
const { isStackOverflowError, } = require('internal/errors');
const { isAsyncFunction, isGeneratorFunction, isAnyArrayBuffer, isArrayBuffer, isArgumentsObject, isBoxedPrimitive, isDataView, isExternal, isMap, isMapIterator, isModuleNamespaceObject, isNativeError, isPromise, isSet, isSetIterator, isWeakMap, isWeakSet, isRegExp, isDate, isTypedArray, isStringObject, isNumberObject, isBooleanObject, isBigIntObject, } = require('internal/util/types');
const assert = require('internal/assert');
const { BuiltinModule } = require('internal/bootstrap/realm');
const { validateObject, validateString, kValidateObjectAllowArray, } = require('internal/validators');
let hexSlice;
let internalUrl;
...
const StringPrototypePadStart = String.prototype.padStart.call.bind(String.prototype.padStart);
    ...
    if (arguments.length > 1) {
        ...
        if (typeof opts === 'boolean') {
            ...
            for (let i = 0; i < optKeys.length; ++i) {
                ...
                // functionality.
                if (Object.hasOwn(inspectDefaultOptions, key) ||
                    key === 'stylize') {
                    ctx[key] = opts[key];
                }
...
inspect.styles = Object.assign(CreateNullProtoObject(), {
    special: 'cyan',
    number: 'yellow',
    bigint: 'yellow',
    boolean: 'yellow',
...
function formatRaw(ctx, value, recurseTimes, typedArray) {
    ...
    if (typeof tag !== 'string' ||
        (tag !== '' &&
            (ctx.showHidden ?
                ...
                Object.prototype.propertyIsEnumerable.call.bind(Object.prototype.propertyIsEnumerable))(value, Symbol.toStringTag))) {
        tag = '';
    }
    let base = '';
    ...
    if (Symbol.iterator in value || constructor === null) {
        ...
        if (Array.isArray(value)) {
            ...
            let fallback = '';
            if (constructor === null) {
                fallback = value[Symbol.toStringTag];
    ...
    if (ctx.sorted) {
        ...
        if (extrasType === kObjectType) {
            ...
            sorted.unshift(output, output.length - keys.length, keys.length);
...
function getBoxedBase(value, ctx, keys, constructor, tag) {
    ...
    if (isNumberObject(value)) {
        fn = Number.prototype.valueOf.call.bind(Number.prototype.valueOf);
        type = 'Number';
    }
    else if (isStringObject(value)) {
        fn = String.prototype.valueOf.call.bind(String.prototype.valueOf);
        type = 'String';
        ...
        fn = Boolean.prototype.valueOf.call.bind(Boolean.prototype.valueOf);
        type = 'Boolean';
    }
    else if (isBigIntObject(value)) {
        fn = BigInt.prototype.valueOf.call.bind(BigInt.prototype.valueOf);
        type = 'BigInt';
    }
    else {
        fn = Symbol.prototype.valueOf.call.bind(Symbol.prototype.valueOf);
        type = 'Symbol';
    }
    let base = `[${type}`;
    if (type !== constructor) {
...
const FunctionPrototypeToString = Function.prototype.toString.call.bind(Function.prototype.toString)
...
function getStackString(error) {
    return error.stack ? String(error.stack) : error.toString();
}
...
function improveStack(stack, constructor, name, tag) {
    ...
    if (constructor === null ||
        ...
        let fallback = 'Error';
        if (constructor === null) {
            const start = /^([A-Z][a-z_ A-Z0-9[\]()-]+)(?::|\n {4}at)/.exec(stack) || /^([a-z_A-Z0-9-]*Error)$/.exec(stack);
    ...
    if (stackStart === -1) {
        ...
        if (ctx.colors) {
            ...
            for (let line of lines) {
                const core = coreModuleRegExp.exec(line);
...
function formatArrayBuffer(ctx, value) {
    ...
    if (hexSlice === undefined)
        hexSlice = Function.prototype.call.bind(require('buffer').Buffer.prototype.hexSlice);
