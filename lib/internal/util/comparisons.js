...
const { isAnyArrayBuffer, isArrayBufferView, isDate, isMap, isRegExp, isSet, isNativeError, isBoxedPrimitive, isNumberObject, isStringObject, isBooleanObject, isBigIntObject, isSymbolObject, isFloat32Array, isFloat64Array, isKeyObject, isCryptoKey, } = types;
const { constants: { ONLY_ENUMERABLE, SKIP_SYMBOLS, }, getOwnNonIndexProperties, } = internalBinding('util');
const kStrict = true;
const kLoose = false;
const kNoIterator = 0;
const kIsArray = 1;
...
const ObjectPrototypeValueOf = Object.prototype.valueOf.call.bind(Object.prototype.valueOf)
function isEqualBoxedPrimitive(val1, val2) {
    if (isNumberObject(val1)) {
        return isNumberObject(val2) &&
            Object.is(ObjectPrototypeValueOf(val1), ObjectPrototypeValueOf(val2));
    }
    if (isStringObject(val1)) {
        return isStringObject(val2) &&
            ObjectPrototypeValueOf(val1) === ObjectPrototypeValueOf(val2);
    }
