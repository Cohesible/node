...
const { assertCrypto, customInspectSymbol: kInspect, kEmptyObject, promisify, } = require('internal/util');
assertCrypto();
...
const { kIncomingMessage, _checkIsHttpToken: checkIsHttpToken, } = require('_http_common');
const { kServerResponse, Server: HttpServer, httpServerPreClose, setupConnectionsTracking } = require('_http_server');
const JSStreamSocket = require('internal/js_stream_socket');
const { defaultTriggerAsyncIdScope, symbols: { async_id_symbol, owner_symbol, }, } = require('internal/async_hooks');
const { aggregateTwoErrors, codes: { ERR_HTTP2_ALTSVC_INVALID_ORIGIN, ERR_HTTP2_ALTSVC_LENGTH, ERR_HTTP2_CONNECT_AUTHORITY, ERR_HTTP2_CONNECT_PATH, ERR_HTTP2_CONNECT_SCHEME, ERR_HTTP2_GOAWAY_SESSION, ERR_HTTP2_HEADERS_AFTER_RESPOND, ERR_HTTP2_HEADERS_SENT, ERR_HTTP2_INVALID_INFO_STATUS, ERR_HTTP2_INVALID_ORIGIN, ERR_HTTP2_INVALID_PACKED_SETTINGS_LENGTH, ERR_HTTP2_INVALID_SESSION, ERR_HTTP2_INVALID_SETTING_VALUE, ERR_HTTP2_INVALID_STREAM, ERR_HTTP2_MAX_PENDING_SETTINGS_ACK, ERR_HTTP2_NESTED_PUSH, ERR_HTTP2_NO_MEM, ERR_HTTP2_NO_SOCKET_MANIPULATION, ERR_HTTP2_ORIGIN_LENGTH, ERR_HTTP2_OUT_OF_STREAMS, ERR_HTTP2_PAYLOAD_FORBIDDEN, ERR_HTTP2_PING_CANCEL, ERR_HTTP2_PING_LENGTH, ERR_HTTP2_PUSH_DISABLED, ERR_HTTP2_SEND_FILE, ERR_HTTP2_SEND_FILE_NOSEEK, ERR_HTTP2_SESSION_ERROR, ERR_HTTP2_SETTINGS_CANCEL, ERR_HTTP2_SOCKET_BOUND, ERR_HTTP2_SOCKET_UNBOUND, ERR_HTTP2_STATUS_101, ERR_HTTP2_STATUS_INVALID, ERR_HTTP2_STREAM_CANCEL, ERR_HTTP2_STREAM_ERROR, ERR_HTTP2_STREAM_SELF_DEPENDENCY, ERR_HTTP2_TOO_MANY_CUSTOM_SETTINGS, ERR_HTTP2_TRAILERS_ALREADY_SENT, ERR_HTTP2_TRAILERS_NOT_READY, ERR_HTTP2_UNSUPPORTED_PROTOCOL, ERR_INVALID_ARG_TYPE, ERR_INVALID_ARG_VALUE, ERR_INVALID_CHAR, ERR_INVALID_HTTP_TOKEN, ERR_OUT_OF_RANGE, ERR_SOCKET_CLOSED, }, hideStackFrames, AbortError, } = require('internal/errors');
const { isUint32, validateAbortSignal, validateBoolean, validateBuffer, validateFunction, validateInt32, validateInteger, validateNumber, validateString, validateUint32, } = require('internal/validators');
const fsPromisesInternal = require('internal/fs/promises');
const { utcDate } = require('internal/http');
const { Http2ServerRequest, Http2ServerResponse, onServerStream, } = require('internal/http2/compat');
const { assertIsObject, assertIsArray, assertValidPseudoHeader, assertValidPseudoHeaderResponse, assertValidPseudoHeaderTrailer, assertWithinRange, getAuthority, getDefaultSettings, getSessionState, getSettings, getStreamState, isPayloadMeaningless, kSensitiveHeaders, kSocket, kRequest, kProxySocket, mapToHeaders, MAX_ADDITIONAL_SETTINGS, NghttpError, remoteCustomSettingsToBuffer, sessionName, toHeaderObject, updateOptionsBuffer, updateSettingsBuffer, } = require('internal/http2/util');
const { writeGeneric, writevGeneric, onStreamRead, kAfterAsyncWrite, kMaybeDestroy, kUpdateTimer, kHandle, kSession, kBoundSession, setStreamTimeout, } = require('internal/stream_base_commons');
const { kTimeout } = require('internal/timers');
const { isArrayBufferView } = require('internal/util/types');
const { format } = require('internal/util/inspect');
const { FileHandle } = internalBinding('fs');
const binding = internalBinding('http2');
const { ShutdownWrap, kReadBytesOrError, streamBaseState, } = internalBinding('stream_wrap');
...
function debugStream(id, sessionType, message, ...args) {
    ...
    debug('Http2Stream %s [Http2Session %s]: ' + message, id, sessionName(sessionType), ...args);
}
    ...
    get(session, prop) {
        switch (prop) {
            ...
            default: {
                ...
                return typeof value === 'function' ? value.bind(socket) :
                    value;
            }
        }
    },
...
const validateSettings = hideStackFrames((settings) => {
    ...
    assertWithinRange.withoutStackTrace('initialWindowSize', settings.initialWindowSize, 0, kMaxInt);
...
function setupHandle(socket, type, options) {
    ...
    assert(socket._handle !== undefined, 'Internal HTTP/2 Failure. The socket is not connected. Please ' +
...
class ServerHttp2Session extends Http2Session {
    ...
    altsvc(alt, originOrStream) {
        ...
        if (kQuotedString.exec(alt) === null)
...
class ClientHttp2Session extends Http2Session {
    ...
    request(headers, options) {
        ...
        headers = Object.assign(CreateNullProtoObject(), headers);
        options = { ...options };
        ...
        if (signal) {
            ...
            if (signal.aborted) {
                ...
                stream.once('close', disposable[Symbol.for("Symbol.dispose")]);
            }
        }
...
function processHeaders(oldHeaders, options) {
    ...
    const headers = CreateNullProtoObject();
...
function onFileUnpipe() {
    ...
    if (stream.ownsFd)
        this.source.close().then(undefined, stream.destroy.bind(stream));
    else
...
function doSendFD(session, options, fd, headers, streamOptions, err, stat) {
    ...
    if ((typeof options.statCheck === 'function' &&
        Reflect.apply(options.statCheck, this, [stat, headers, statOptions]) === false) ||
...
class Http2SecureServer extends TLSServer {
    constructor(options, requestListener) {
        ...
        if (options.allowHTTP1 === true) {
            this.headersTimeout = 60000; // Minimum between 60 seconds or requestTimeout
...
Http2Server.prototype[EventEmitter.captureRejectionSymbol] = function (err, event, ...args) {
    switch (event) {
        ...
        default:
            ...
            Reflect.apply(net.Server.prototype[EventEmitter.captureRejectionSymbol], this, args);
    }
};
