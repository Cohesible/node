...
const advanced = {
    ...
    *parseChannelMessages(channel, readData) {
        ...
        while (messageBufferHead.length >= 4) {
            ...
            const fullMessageSize = (messageBufferHead[0] << 24 |
                messageBufferHead[1] << 16 |
                messageBufferHead[2] << 8 |
                messageBufferHead[3]) + 4;
            if (channel[kMessageBufferSize] < fullMessageSize)
                break;
            ...
            const deserializer = new ChildProcessDeserializer(concatenatedBuffer.subarray(4, fullMessageSize));
