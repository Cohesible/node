'use strict';
const { codes: { ERR_CRYPTO_CUSTOM_ENGINE_NOT_SUPPORTED, ERR_INVALID_ARG_TYPE, ERR_INVALID_ARG_VALUE, }, } = require('internal/errors');
const { kEmptyObject, } = require('internal/util');
const { isArrayBufferView, } = require('internal/util/types');
const { validateBuffer, validateInt32, validateObject, validateString, } = require('internal/validators');
const { toBuf, } = require('internal/crypto/util');
const { crypto: { TLS1_2_VERSION, TLS1_3_VERSION, }, } = internalBinding('constants');
...
function configSecureContext(context, options = kEmptyObject, name = 'options') {
    ...
    const { ca, cert, ciphers = getDefaultCiphers(), clientCertEngine, crl, dhparam, ecdhCurve = getDefaultEcdhCurve(), key, passphrase, pfx, privateKeyIdentifier, privateKeyEngine, sessionIdContext, sessionTimeout, sigalgs, ticketKeys, } = options;
    ...
    const { cipherList, cipherSuites, } = processCiphers(ciphers, `${name}.ciphers`);
    if (cipherSuites !== '')
    ...
    if (privateKeyIdentifier !== undefined && privateKeyIdentifier !== null) {
        ...
        if (typeof privateKeyIdentifier === 'string' &&
            ...
            throw new ERR_INVALID_ARG_TYPE(`${name}.privateKeyIdentifier`, ['string', 'null', 'undefined'], privateKeyIdentifier);
        }
        else {
            throw new ERR_INVALID_ARG_TYPE(`${name}.privateKeyEngine`, ['string', 'null', 'undefined'], privateKeyEngine);
        }
    }
