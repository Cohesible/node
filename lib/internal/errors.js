...
const ErrorPrototypeToString = Error.prototype.toString.call.bind(Error.prototype.toString);
/**
...
function prepareStackTraceCallback(globalThis, error, trace) {
    ...
    if (typeof Error.prepareStackTrace === 'function') {
        return Error.prepareStackTrace(error, trace);
    }
    ...
    switch (expectedLength) {
        case 0: {
            class NodeError extends Base {
                code = key;
                constructor(...args) {
                    assert(args.length === 0, `Code: ${key}; The provided arguments length (${args.length}) does not ` +
        ...
        default: {
            class NodeError extends Base {
                code = key;
                constructor(...args) {
                    assert(args.length === expectedLength, `Code: ${key}; The provided arguments length (${args.length}) does not ` +
                        `match the required ones (${expectedLength}).`);
                    args.unshift(msg);
...
function getMessage(key, args, self) {
    ...
    assert(expectedLength === args.length, `Code: ${key}; The provided arguments length (${args.length}) does not ` +
        `match the required ones (${expectedLength}).`);
    if (args.length === 0)
        return msg;
    args.unshift(msg);
...
const fatalExceptionStackEnhancers = {
    ...
    afterInspector(error) {
        ...
        if (isWindows) {
            ...
            const ver = info[2].split('.').map(Number);
            if (ver[0] !== 10 || ver[2] < 14393) {
                useColors = false;
            }
        }
        const { inspect, inspectDefaultOptions: { colors: defaultColors, }, } = lazyInternalUtilInspect();
...
const { privateSymbols: { arrow_message_private_symbol, }, } = internalBinding('util');
...
function hideInternalStackFrames(error) {
    overrideStackTrace.set(error, (error, stackFrames) => {
        ...
        if (typeof stackFrames === 'object') {
            frames = stackFrames.filter((frm) => !(frm.getFileName() || '').startsWith('node:internal'));
        }
        frames.unshift(error);
        return frames.join('\n    at ');
    });
}
...
E('ERR_ACCESS_DENIED', 'Access to this API has been restricted. Permission: %s', Error);
...
E('ERR_DOMAIN_CALLBACK_NOT_AVAILABLE', 'A callback was registered through ' +
    ...
    'exclusive with using the `domain` module', Error);
E('ERR_DOMAIN_CANNOT_SET_UNCAUGHT_EXCEPTION_CAPTURE', 'The `domain` module is in use, which is mutually exclusive with calling ' +
...
E('ERR_FEATURE_UNAVAILABLE_ON_PLATFORM', 'The feature %s is unavailable on the current platform' +
...
E('ERR_HTTP2_INVALID_PSEUDOHEADER', '"%s" is an invalid pseudoheader or is used incorrectly', TypeError, HideStackFramesError);
...
E('ERR_HTTP2_PSEUDOHEADER_NOT_ALLOWED', 'Cannot set HTTP/2 pseudo-headers', TypeError, HideStackFramesError);
...
E('ERR_HTTP2_TRAILERS_NOT_READY', 'Trailing headers cannot be sent until after the wantTrailers event is ' +
    'emitted', Error);
...
E('ERR_IMPORT_ATTRIBUTE_MISSING', 'Module "%s" needs an import attribute of "%s: %s"', TypeError);
E('ERR_IMPORT_ATTRIBUTE_TYPE_INCOMPATIBLE', 'Module "%s" is not of type "%s"', TypeError);
E('ERR_IMPORT_ATTRIBUTE_UNSUPPORTED', 'Import attribute "%s" with value "%s" is not supported', TypeError);
E('ERR_INCOMPATIBLE_OPTION_PAIR', 'Option "%s" cannot be used in combination with option "%s"', TypeError, HideStackFramesError);
E('ERR_INPUT_TYPE_NOT_ALLOWED', '--input-type can only be used with string ' +
    'input via --eval, --print, or STDIN', Error);
E('ERR_INSPECTOR_ALREADY_ACTIVATED', 'Inspector is already activated. Close it with inspector.close() ' +
    'before activating it again.', Error);
...
E('ERR_INVALID_ARG_TYPE', (name, expected, actual) => {
    ...
    if (name.endsWith(' argument')) {
        ...
        const type = name && name.includes('.') ? 'property' : 'argument';
        msg += `"${name}" ${type} `;
    }
    msg += 'must be ';
    const types = [];
...
E('ERR_INVALID_PACKAGE_TARGET', (pkgPath, key, target, isImport = false, base = undefined) => {
    ...
    return `Invalid "${isImport ? 'imports' : 'exports'}" target ${JSON.stringify(target)} defined for '${key}' in the package config ${pkgPath}package.json${base ? ` imported from ${base}` : ''}${relError ?
        '; targets must start with "./"' : ''}`;
}, Error);
...
E('ERR_INVALID_SYNC_FORK_INPUT', 'Asynchronous forks do not support ' +
    'Buffer, TypedArray, DataView or string input: %s', TypeError);
...
E('ERR_LOADER_CHAIN_INCOMPLETE', '"%s" did not call the next hook in its chain and did not' +
    ' explicitly signal a short circuit. If this is intentional, include' +
    ' `shortCircuit: true` in the hook\'s return.', Error);
E('ERR_MANIFEST_ASSERT_INTEGRITY', (moduleURL, realIntegrities) => {
    ...
    if (realIntegrities.size) {
        const sri = Array.from(realIntegrities.entries(), ({ 0: alg, 1: dgs }) => `${alg}-${dgs}`).join(' ');
...
E('ERR_MANIFEST_DEPENDENCY_MISSING', 'Manifest resource %s does not list %s as a dependency specifier for ' +
    'conditions: %s', Error);
...
E('ERR_MISSING_ARGS', (...args) => {
    ...
    const wrap = (a) => `"${a}"`;
    args = args.map((a) => (Array.isArray(a) ? a.map(wrap).join(' or ') :
        wrap(a)));
...
E('ERR_NAPI_INVALID_DATAVIEW_ARGS', 'byte_offset + byte_length should be less than or equal to the size in ' +
    'bytes of the array passed in', RangeError);
...
E('ERR_NETWORK_IMPORT_BAD_RESPONSE', "import '%s' received a bad response: %s", Error);
E('ERR_NETWORK_IMPORT_DISALLOWED', "import of '%s' by %s is not supported: %s", Error);
...
E('ERR_TLS_ALPN_CALLBACK_WITH_PROTOCOLS', 'The ALPNCallback and ALPNProtocols TLS options are mutually exclusive', TypeError);
...
E('ERR_UNAVAILABLE_DURING_EXIT', 'Cannot call function in process exit ' +
    'handler', Error);
E('ERR_UNCAUGHT_EXCEPTION_CAPTURE_ALREADY_SET', '`process.setupUncaughtExceptionCapture()` was called while a capture ' +
    'callback was already active', Error);
...
E('ERR_UNSUPPORTED_RESOLVE_REQUEST', 'Failed to resolve module specifier "%s" from "%s": Invalid relative URL or base scheme is not hierarchical.', TypeError);
...
E('ERR_WORKER_INVALID_EXEC_ARGV', (errors, msg = 'invalid execArgv flags') => `Initiated Worker with ${msg}: ${errors.join(', ')}`, Error);
E('ERR_WORKER_NOT_RUNNING', 'Worker instance not running', Error);
E('ERR_WORKER_OUT_OF_MEMORY', 'Worker terminated due to reaching memory limit: %s', Error);
E('ERR_WORKER_PATH', (filename) => 'The worker script or module filename must be an absolute path or a ' +
    'relative path starting with \'./\' or \'../\'.' +
    (filename.startsWith('file://') ?
        ' Wrap file:// URLs with `new URL`.' : '') +
    (filename.startsWith('data:text/javascript') ?
        ' Wrap data: URLs with `new URL`.' : '') +
    ` Received "${filename}"`, TypeError);
