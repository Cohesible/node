...
const { setInterval: pSetInterval, setTimeout: pSetTimeout, } = require('timers/promises');
...
const { exitCodes: { kGenericUserError, kNoFailure, kInvalidCommandLineArgument, }, } = internalBinding('errors');
async function portIsFree(host, port, timeout = 3000) {
    if (port === 0)
        return; // Binding to a random port.
...
async function runScript(script, scriptArgs, inspectHost, inspectPort, childPrint) {
    ...
    return new Promise((resolve) => {
        function waitForListenHint(text) {
            output += text;
            const debug = debugRegex.exec(output);
            if (debug) {
                const host = debug[1];
...
function createAgentProxy(domain, client) {
    ...
    agent.then = (then, _catch) => {
        ...
        return Promise.resolve(descriptor).then(then, _catch);
    };
...
class NodeInspector {
    ...
    childPrint(text, which) {
        const lines = (this.#stdioBuffers[which] + text).split(/\r\n|\r|\n/g);
...
function parseArgv(args) {
    ...
    let script = target;
    let scriptArgs = args;
    const hostMatch = /^([^:]+):(\d+)$/.exec(target);
    const portMatch = /^--port=(\d+)$/.exec(target);
    if (hostMatch) {
    ...
    else if (args.length === 1 && /^\d+$/.exec(args[0]) !== null &&
        target === '-p') {
