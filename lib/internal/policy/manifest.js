'use strict';
const { ERR_MANIFEST_ASSERT_INTEGRITY, ERR_MANIFEST_INVALID_RESOURCE_FIELD, ERR_MANIFEST_INVALID_SPECIFIER, ERR_MANIFEST_UNKNOWN_ONERROR, } = require('internal/errors').codes;
        ...
        if (dependencies === kFallThrough ||
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                if (key.endsWith('*')) {
                    const target = /^([^*]*)\*([^*]*)$/.exec();
                    if (!target) {
                        throw new ERR_MANIFEST_INVALID_SPECIFIER(this.href, `${target}, pattern needs to have a single trailing "*" in target`);
                    }
    ...
    _resolveAlreadyNormalized(normalizedSpecifier, conditions, manifest) {
        ...
        if (dependencies !== undefined && typeof dependencies === 'object') {
            ...
            if (normalized !== true) {
                ...
                Object.freeze(normalizedDependencyMap);
            ...
            if (normalizedSpecifier in dependencies === true) {
                ...
                else if (kRelativeURLStringPattern.exec(to) !== null) {
...
class Manifest {
    ...
    constructor(obj, manifestHREF) {
        ...
        const jsonResourcesEntries = Object.entries(obj.resources ?? { __proto__: null });
        const jsonScopesEntries = Object.entries(obj.scopes ?? { __proto__: null });
    ...
    getDependencyMapper(requester) {
        ...
        const instance = (dependencies.has(requesterHREF) ?
            ...
            this.getScopeDependencyMapper(requesterHREF, true)) ?? this.#defaultDependencies;
        return {
    ...
    mightAllow(url, onreact) {
        ...
        while (scope !== null) {
            ...
            const nextScope = findScopeHREF(new URL('..', scope), this.#scopeIntegrities, false);
    ...
    assertIntegrity(url, content) {
        ...
        function processEntry(href) {
            ...
            if (typeof integrityEntries === 'string') {
                const sri = Object.freeze(SRI.parse(integrityEntries));
        ...
        if (integrities.has(href)) {
            ...
            if (integrityEntries === true)
                return true;
            if (Array.isArray(integrityEntries)) {
...
function canonicalizeSpecifier(specifier, base) {
    try {
        if (kRelativeURLStringPattern.exec(specifier) !== null) {
...
const emptyOrProtocolOrResolve = (resourceHREF, base) => {
    ...
    if (resourceHREF.endsWith(':')) {
        ...
        if (/^[a-zA-Z][a-zA-Z+\-.]*:$/.exec(resourceHREF) !== null) {
            return resourceHREF;
        }
    }
...
const resolve = (originalHREF, base) => {
    parsedURLs = parsedURLs ?? new Map();
    if (parsedURLs.has(originalHREF)) {
        return parsedURLs.get(originalHREF);
    }
    else if (kRelativeURLStringPattern.exec(originalHREF) !== null) {
