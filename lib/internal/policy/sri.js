'use strict';
const { ERR_SRI_PARSE, } = require('internal/errors').codes;
const kWSP = '[\\x20\\x09]';
...
const parse = (str) => {
    ...
    let match;
    const entries = [];
    while ((match = kSRIPattern.exec(str)) !== null) {
    ...
    if (prevIndex !== str.length) {
        if (kAllWSP.exec(str.slice(prevIndex)) === null) {
            throw new ERR_SRI_PARSE(str, str[prevIndex], prevIndex);
        }
    }
    return Object.setPrototypeOf(entries, Array.prototype);
};
module.exports = {
    parse,
};
