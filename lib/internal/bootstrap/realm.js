...
const synapseBuiltins = new Set(['esbuild', 'typescript']);
// Modules that will only be enabled at run time.
const experimentalModuleList = new Set();
// Set up process.binding() and process._linkedBinding().
{
    const bindingObj = CreateNullProtoObject();
...
const { builtinIds, compileFunction, setInternalLoaders, } = internalBinding('builtins');
...
let canBeRequiredByUsersWithoutSchemeList = new Set(publicBuiltinIds.filter((id) => !schemelessBlockList.has(id)));
/**
...
class BuiltinModule {
    ...
    static setRealmAllowRequireByUsers(ids) {
        canBeRequiredByUsersList =
            new Set(ids.filter((id) => publicBuiltinIds.includes(id)));
        canBeRequiredByUsersWithoutSchemeList =
            new Set(ids.filter((id) => !schemelessBlockList.has(id)));
    }
    ...
    static isBuiltin(id) {
        return BuiltinModule.canBeRequiredWithoutScheme(id) || (typeof id === 'string' && id.startsWith('node:') &&
            BuiltinModule.canBeRequiredByUsers(id.slice(5)));
    }
    ...
    static isSynapseBuiltin(id) {
        return synapseBuiltins.has(id);
    }
...
function setupPrepareStackTrace() {
    const { setEnhanceStackForFatalException, setPrepareStackTraceCallback, } = internalBinding('errors');
    const { prepareStackTraceCallback, ErrorPrepareStackTrace, fatalExceptionStackEnhancers: { beforeInspector, afterInspector, }, } = requireBuiltin('internal/errors');
