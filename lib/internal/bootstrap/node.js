...
const { constants: { kExitCode, kExiting, kHasExitCode, }, privateSymbols: { exit_info_private_symbol, }, } = internalBinding('util');
setupProcessObject();
setupGlobalProxy();
setupBuffer();
process.domain = null;
...
{
    const { onGlobalUncaughtException, setUncaughtExceptionCaptureCallback, hasUncaughtExceptionCaptureCallback, } = require('internal/process/execution');
...
{
    const { nextTick, runNextTicks, nextImmediateTick } = setupTaskQueue();
    process.nextTick = nextTick;
    process.nextImmediateTick = nextImmediateTick;
...
{
    const { getSourceMapsEnabled, setSourceMapsEnabled, maybeCacheGeneratedSourceMap, } = require('internal/source_map/source_map_cache');
