...
const { ERR_INSPECTOR_NOT_AVAILABLE, } = require('internal/errors').codes;
...
const { getStringWidth, inspect, } = require('internal/util/inspect');
...
function setupPreview(repl, contextSymbol, bufferSymbol, active) {
    ...
    function isInStrictMode(repl) {
        return repl.replMode === REPL_MODE_STRICT || process.execArgv.map((e) => e.toLowerCase().replaceAll('_', '-')).includes('--use-strict');
    }
