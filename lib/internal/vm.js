'use strict';
const { ContextifyScript, compileFunction, } = internalBinding('contextify');
const { runInContext, } = ContextifyScript.prototype;
const { vm_dynamic_import_default_internal, vm_dynamic_import_main_context_default, vm_dynamic_import_no_callback, vm_dynamic_import_use_filename } = internalBinding('symbols');
const { validateFunction, } = require('internal/validators');
const { privateSymbols: { contextify_context_private_symbol, }, } = internalBinding('util');
/**
 ...
 * ID.
 */
function getHostDefinedOptionId(importModuleDynamically, hint, filename) {
    ...
    if (filename) {
        return vm_dynamic_import_use_filename
    }
    return Symbol(hint);
}
/**
 ...
 */
function registerImportModuleDynamically(referrer, importModuleDynamically, name) {
 ...
 */
function internalCompileFunction(code, filename, lineOffset, columnOffset, cachedData, produceCachedData, parsingContext, contextExtensions, params, hostDefinedOptionId, importModuleDynamically) {
    const result = compileFunction(code, filename, lineOffset, columnOffset, cachedData, produceCachedData, parsingContext, contextExtensions, params, hostDefinedOptionId);
 ...
 */
function makeContextifyScript(code, filename, lineOffset, columnOffset, cachedData, produceCachedData, parsingContext, hostDefinedOptionId, importModuleDynamically) {
    let script;
    // Calling `ReThrow()` on a native TryCatch does not generate a new
    // abort-on-uncaught-exception check. A dummy try/catch in JS land
    // protects against that.
    try { // eslint-disable-line no-useless-catch
        script = new ContextifyScript(code, filename, lineOffset, columnOffset, cachedData, produceCachedData, parsingContext, hostDefinedOptionId);
    }
    catch (e) {
...
function runScriptInThisContext(script, displayErrors, breakOnFirstLine) {
    return Reflect.apply(runInContext, script, [
