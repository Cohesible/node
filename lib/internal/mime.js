'use strict';
const { ERR_INVALID_MIME_SYNTAX, } = require('internal/errors').codes;
...
function toASCIILower(str) {
    ...
    for (let i = 0; i < str.length; i++) {
        const char = str[i];
        result += char >= 'A' && char <= 'Z' ? char.toLowerCase() :
            char;
    }
    return result;
}
    ...
    #parse() {
        ...
        while (position < endOfSource) {
            ...
            if (char === '"') {
                ...
                // unescape
                const inside = insideMatch[1] || insideMatch[2] ? insideMatch[0].slice(0, -1) :
                    insideMatch[0];
