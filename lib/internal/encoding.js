'use strict';
const { ERR_ENCODING_NOT_SUPPORTED, ERR_INVALID_ARG_TYPE, ERR_INVALID_THIS, ERR_NO_ICU, } = require('internal/errors').codes;
...
const { getConstructorOf, customInspectSymbol: inspect, kEmptyObject, kEnumerableProperty, } = require('internal/util');
const { isAnyArrayBuffer, isArrayBufferView, isUint8Array, } = require('internal/util/types');
const { validateString, validateObject, kValidateObjectAllowObjectsAndNull, } = require('internal/validators');
const binding = internalBinding('encoding_binding');
const { encodeInto, encodeIntoResults, encodeUtf8String, decodeUTF8, } = binding;
...
Object.defineProperties(TextEncoder.prototype, {
    ...
    [Symbol.toStringTag]: { configurable: true, value: 'TextEncoder' },
});
const TextDecoder = internalBinding('config').hasIntl ?
    makeTextDecoderICU() :
    makeTextDecoderJS();
function makeTextDecoderICU() {
    const { decode: _decode, getConverter, } = internalBinding('icu');
    class TextDecoder {
