...
class FilesWatcher extends EventEmitter {
    ...
    #isPathWatched(path) {
        ...
        for (const { 0: watchedPath, 1: watcher } of this.#watchers.entries()) {
            if (watcher.recursive && path.startsWith(watchedPath)) {
                return true;
            }
        }
        return false;
    ...
    #removeWatchedChildren(path) {
        for (const { 0: watchedPath, 1: watcher } of this.#watchers.entries()) {
            if (path !== watchedPath && watchedPath.startsWith(path)) {
    ...
    filterFile(file, owner) {
        ...
        if (owner) {
            const owners = this.#depencencyOwners.get(file) ?? new Set();
        ...
        child.on('message', (message) => {
            try {
                if (Array.isArray(message['watch:require'])) {
                    message['watch:require'].forEach((file) => this.filterFile(file, key));
                }
                if (Array.isArray(message['watch:import'])) {
                    message['watch:import'].forEach((file) => this.filterFile(fileURLToPath(file), key));
                }
            }
            catch {
