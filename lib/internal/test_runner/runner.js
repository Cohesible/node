...
const { codes: { ERR_INVALID_ARG_TYPE, ERR_INVALID_ARG_VALUE, ERR_TEST_FAILURE, ERR_OUT_OF_RANGE, }, } = require('internal/errors');
const { validateArray, validateBoolean, validateFunction, validateObject, validateInteger, } = require('internal/validators');
const { getInspectPort, isUsingInspector, isInspectorMessage } = require('internal/util/inspector');
const { isRegExp } = require('internal/util/types');
const { kEmptyObject } = require('internal/util');
const { kEmitMessage } = require('internal/test_runner/tests_stream');
const { createTestTree } = require('internal/test_runner/harness');
const { kAborted, kCancelledByParent, kSubtestsFailed, kTestCodeFailure, kTestTimeoutFailure, Test, } = require('internal/test_runner/test');
const { convertStringToRegExp, countCompletedTest, kDefaultPattern, shouldColorizeTestFiles, } = require('internal/test_runner/utils');
const { Glob } = require('internal/fs/glob');
const { once } = require('events');
const { triggerUncaughtException, exitCodes: { kGenericUserError }, } = internalBinding('errors');
...
function createTestFileList() {
    ...
    const patterns = hasUserSuppliedPattern ? process.argv.slice(1) : [kDefaultPattern];
    const glob = new Glob(patterns, {
        cwd,
        exclude: (name) => name === 'node_modules'
    });
    const results = glob.globSync();
    if (hasUserSuppliedPattern && results.length === 0 && glob.matchers.every((m) => !m.hasMagic())) {
...
function filterExecArgv(arg, i, arr) {
    return !kFilterArgs.includes(arg) &&
        !kFilterArgValues.some((p) => arg === p || (i > 0 && arr[i - 1] === p) || arg.startsWith(`${p}=`));
}
function getRunArgs(path, { forceExit, inspectPort, testNamePatterns, only }) {
    const argv = process.execArgv.filter(filterExecArgv);
    if (forceExit === true) {
        argv.push('--test-force-exit');
    }
    if (isUsingInspector()) {
        argv.push(`--inspect-port=${getInspectPort(inspectPort)}`);
    }
    if (testNamePatterns != null) {
        testNamePatterns.forEach((pattern) => argv.push(`--test-name-pattern=${pattern}`));
    }
    if (only === true) {
        argv.push('--test-only');
    }
...
const kV8HeaderLength = v8Header.length;
...
class FileTest extends Test {
    ...
    #checkNestedComment(comment) {
        ...
        if (firstSpaceIndex === -1)
            return false;
        const secondSpaceIndex = comment.indexOf(' ', firstSpaceIndex + 1);
        return secondSpaceIndex === -1 && kDiagnosticsFilterArgs.includes(comment.slice(0, firstSpaceIndex));
    }
    ...
    #drainReportBuffer() {
        if (this.#reportBuffer.length > 0) {
            this.#reportBuffer.forEach((ast) => this.#handleReportItem(ast));
            this.#reportBuffer = [];
        }
    }
    addToReport(item) {
    ...
    parseMessage(readData) {
        ...
        if (partialV8Header) {
            ...
            readData = readData.subarray(0, dataLength - 1);
            dataLength--;
        }
        if (this.#rawBuffer[0] && this.#rawBuffer[0].length < kSerializedSizeHeader) {
            this.#rawBuffer[0] = Buffer.concat([this.#rawBuffer[0], readData]);
        }
        else {
            this.#rawBuffer.push(readData);
        }
        this.#rawBufferSize += dataLength;
        this.#proccessRawBuffer();
        if (partialV8Header) {
            this.#rawBuffer.push(v8Header.subarray(0, 1));
            this.#rawBufferSize++;
        }
    }
    #drainRawBuffer() {
    ...
    #proccessRawBuffer() {
        ...
        while (bufferHead && headerIndex !== 0) {
            ...
            this.#rawBufferSize -= nonSerializedData.length;
            if (headerIndex === -1) {
                this.#rawBuffer.shift();
            }
            else {
                this.#rawBuffer[0] = bufferHead.subarray(headerIndex);
            }
        ...
        while (bufferHead?.length >= kSerializedSizeHeader) {
            ...
            const fullMessageSize = (bufferHead[kV8HeaderLength] << 24 |
                bufferHead[kV8HeaderLength + 1] << 16 |
                bufferHead[kV8HeaderLength + 2] << 8 |
                bufferHead[kV8HeaderLength + 3]) + kSerializedSizeHeader;
            ...
            const deserializer = new DefaultDeserializer(concatenatedBuffer.subarray(kSerializedSizeHeader, fullMessageSize));
            bufferHead = concatenatedBuffer.subarray(fullMessageSize);
            this.#rawBufferSize = bufferHead.length;
...
function runTestFile(path, filesWatcher, opts) {
    ...
    const subtest = opts.root.createSubtest(FileTest, path, { signal: opts.signal }, async (t) => {
        ...
        const { 0: { 0: code, 1: signal } } = await Promise.all([
...
function watchFiles(testFiles, opts) {
    ...
    const watcher = new FilesWatcher({ debounce: 200, mode: 'filter', signal: opts.signal });
    const filesWatcher = { watcher, runningProcesses, runningSubtests };
...
function run(options = kEmptyObject) {
    ...
    const { concurrency, timeout, signal, files, forceExit, inspectPort, watch, setup, only, } = options;
    if (files != null) {
    ...
    if (testNamePatterns != null) {
        ...
        testNamePatterns = testNamePatterns.map((value, i) => {
            if (isRegExp(value)) {
                return value;
            }
    ...
    const runFiles = () => {
        ...
        return Promise.allSettled(testFiles.map((path) => {
            const subtest = runTestFile(path, filesWatcher, opts);
            filesWatcher?.runningSubtests.set(path, subtest);
            return subtest;
        }));
    };
    Promise.resolve(setup?.(root.reporter)).then(runFiles).then(postRun);
    return root.reporter;
}
module.exports = {
