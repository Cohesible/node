...
const { validateAbortSignal, validateArray, validateNumber, } = require('internal/validators');
const { AbortError, codes: { ERR_INVALID_STATE, ERR_INVALID_ARG_VALUE }, } = require('internal/errors');
...
class MockTimers {
    ...
    #createDate() {
        ...
        MockDate.toString = function toString() {
            return MockDate[kMock].#nativeDateDescriptor.value.toString();
        };
    ...
    #toggleEnableTimers(activate) {
        ...
        this.#timersInContext.forEach((timer) => target[timer]());
    ...
    enable(options = { __proto__: null, apis: SUPPORTED_APIS, now: 0 }) {
        ...
        internalOptions.apis.forEach((timer) => {
        ...
        if (this.#isValidDateWithGetTime(internalOptions.now)) {
            this.#now = internalOptions.now.getTime();
        }
