'use strict';
const { copyFileSync, mkdirSync, mkdtempSync, opendirSync, readFileSync, } = require('fs');
...
class CoverageLine {
    constructor(line, src, startOffset) {
        const newlineLength = kLineEndingRegex.exec(src)?.[0].length ?? 0;
        this.line = line;
        this.src = src;
...
class TestCoverage {
    ...
    getLines(fileUrl, source) {
        ...
        const linesWithBreaks = source.split(kLineSplitRegex);
        let ignoreCount = 0;
        let enabled = true;
        let offset = 0;
        const lines = linesWithBreaks.map((line, i) => {
            ...
            if (!coverageLine.ignore) {
                ...
                // comments.
                const match = kIgnoreRegex.exec(line);
                if (match !== null) {
            ...
            const match = kStatusRegex.exec(line);
    ...
    getCoverageFromDirectory() {
        ...
        try {
            dir = opendirSync(this.coverageDirectory);
            for (let entry; (entry = dir.readSync()) !== null;) {
                if (kCoverageFileRegex.exec(entry.name) === null) {
                    continue;
                }
    ...
    mapCoverageWithSourceMap(coverage) {
        ...
        for (let i = 0; i < result.length; ++i) {
            ...
            const sourceMap = new SourceMap(data, { lineLengths });
            for (let j = 0; j < functions.length; ++j) {
                ...
                for (let k = 0; k < ranges.length; ++k) {
                    ...
                    if (!startEntry.originalSource && endEntry.originalSource &&
                        ...
                        startEntry = { originalSource, originalLine, originalColumn };
                    }
                    ...
                    newRanges.push({
                        startOffset: mappedStartOffset, endOffset: mappedEndOffset, count
                    });
                }
                if (!newUrl) {
                ...
                const newScript = newResult.get(newUrl) ?? { url: newUrl, functions: [] };
                newScript.functions.push({ functionName, ranges: newRanges, isBlockCoverage });
...
function shouldSkipFileCoverage(url) {
    ...
    // observed on Windows.
    return url.includes('/node_modules/') || !url.startsWith('file:');
}
...
function mergeCoverageScripts(oldScript, newScript) {
    ...
    for (let i = 0; i < newScript.functions.length; ++i) {
        ...
        if (!found) {
            ...
            oldScript.functions.push(newFn);
        }
    }
}
