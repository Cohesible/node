...
const { codes: { ERR_TEST_FAILURE, }, } = require('internal/errors');
const { exitCodes: { kGenericUserError } } = internalBinding('errors');
const { kEmptyObject } = require('internal/util');
const { kCancelledByParent, Test, Suite } = require('internal/test_runner/test');
const { parseCommandLine, reporterScope, setupTestReporters, shouldColorizeTestFiles, } = require('internal/test_runner/utils');
...
function createTestTree(options = kEmptyObject) {
    return setup(new Test({ ...options, name: '<root>' }));
}
...
function setup(root) {
    ...
    root.harness = {
        ...
        watching: false,
        coverage: collectCoverage.bind(null, root, coverage),
        resetCounters() {
