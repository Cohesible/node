...
const { codes: { ERR_INVALID_ARG_TYPE, ERR_TEST_FAILURE, }, AbortError, } = require('internal/errors');
const { MockTracker } = require('internal/test_runner/mock/mock');
const { TestsStream } = require('internal/test_runner/tests_stream');
const { createDeferredCallback, countCompletedTest, isTestFailureError, parseCommandLine, } = require('internal/test_runner/utils');
const { createDeferredPromise, kEmptyObject, once: runOnce, } = require('internal/util');
const { isPromise } = require('internal/util/types');
const { validateAbortSignal, validateNumber, validateOneOf, validateUint32, } = require('internal/validators');
...
const { forceExit, sourceMaps, testNamePatterns, testOnlyFlag, } = parseCommandLine();
...
function stopTest(timeout, signal) {
    ...
    if (timeout === kDefaultTimeout) {
        disposeFunction = abortListener[Symbol.for("Symbol.dispose")];
    }
    else {
        ...
        disposeFunction = () => {
            abortListener[Symbol.for("Symbol.dispose")]();
            timer[Symbol.for("Symbol.dispose")]();
        };
    }
    Object.defineProperty(deferred.promise, Symbol.for("Symbol.dispose"), {
        configurable: true,
        writable: true,
...
class TestContext {
    ...
    before(fn, options) {
        this.#test
            .createHook('before', fn, { ...options, hookType: 'before', loc: getCallerLocation() });
    }
    after(fn, options) {
        this.#test
            .createHook('after', fn, { ...options, hookType: 'after', loc: getCallerLocation() });
    }
    beforeEach(fn, options) {
        this.#test
            .createHook('beforeEach', fn, { ...options, hookType: 'beforeEach', loc: getCallerLocation() });
    }
    afterEach(fn, options) {
        this.#test
            .createHook('afterEach', fn, { ...options, hookType: 'afterEach', loc: getCallerLocation() });
    }
}
class SuiteContext {
    #suite;
...
class Test extends AsyncResource {
    ...
    constructor(options) {
        ...
        this.outerSignal?.addEventListener('abort', this.#abortHandler, { [kResistStopPropagation]: true });
        this.fn = fn;
        ...
        if (!testOnlyFlag && (only || this.runOnlySubtests)) {
            const warning = "'only' and 'runOnly' require the --test-only command-line option.";
        ...
        if (loc === undefined || kFilename === undefined) {
            ...
            if (this.loc.file.startsWith('file://')) {
                this.loc.file = fileURLToPath(this.loc.file);
            }
        }
    }
    matchesTestNamePatterns() {
        const matchesByNameOrParent = testNamePatterns.some((re) => re.exec(this.name) !== null) ||
            this.parent?.matchesTestNamePatterns();
        if (matchesByNameOrParent)
            return true;
        const testNameWithAncestors = this.getTestNameWithAncestors().trim();
        if (!testNameWithAncestors)
            return false;
        return testNamePatterns.some((re) => re.exec(testNameWithAncestors) !== null);
    }
    /**
    ...
    #cancel(error) {
        ...
        this.fail(error ||
            new ERR_TEST_FAILURE('test did not finish before its parent and was cancelled', kCancelledByParent));
        this.cancelled = true;
    ...
    async runHook(hook, args) {
        ...
        try {
            await this.hooks[hook].reduce(async (prev, hook) => {
                await prev;
                await hook.run(args);
                if (hook.error) {
                    throw hook.error;
    ...
    async run() {
        ...
        try {
            ...
            if (this.fn.length === runArgs.length - 1) {
                ...
                if (isPromise(ret)) {
                    this.fail(new ERR_TEST_FAILURE('passed a callback but also returned a Promise', kCallbackAndPromisePresent));
            ...
            catch { /* test is already failing, let's ignore the error */ }
            try {
                await after();
            }
            catch { /* Ignore error. */ }
            ...
            stopPromise?.[Symbol.for("Symbol.dispose")]();
    ...
    getReportDetails() {
        let directive;
        const details = { duration_ms: this.duration() };
        if (this.skipped) {
...
class Suite extends Test {
    ...
    constructor(options) {
        ...
        try {
            ...
            runArgs.push(...args);
            this.buildSuite = Promise.resolve(Reflect.apply(this.runInAsyncScope, this, runArgs)).then(undefined, (err) => {
                this.fail(new ERR_TEST_FAILURE(err, kTestCodeFailure));
            }).finally(() => this.postBuild());
        }
        catch (err) {
            this.fail(new ERR_TEST_FAILURE(err, kTestCodeFailure));
    ...
    async run() {
        ...
        try {
            ...
            const promise = Promise.all(subtests.map((subtests) => subtests.start()));
            ...
            catch { /* suite is already failing */ }
            if (isTestFailureError(err)) {
                this.fail(err);
            }
            else {
