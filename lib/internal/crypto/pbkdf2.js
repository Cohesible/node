...
const { PBKDF2Job, kCryptoJobAsync, kCryptoJobSync, } = internalBinding('crypto');
const { validateFunction, validateInt32, validateString, } = require('internal/validators');
const { getArrayBufferOrView, normalizeHashName, kKeyObject, } = require('internal/crypto/util');
...
function pbkdf2(password, salt, iterations, keylen, digest, callback) {
    ...
    job.ondone = (err, result) => {
        if (err !== undefined)
            return callback.call(job, err);
        const buf = Buffer.from(result);
        return callback.call(job, null, buf);
    };
    job.run();
}
...
async function pbkdf2DeriveBits(algorithm, baseKey, length) {
    ...
    let result;
    try {
        result = await pbkdf2Promise(raw, salt, iterations, length / 8, normalizeHashName(hash.name));
    }
    catch (err) {
