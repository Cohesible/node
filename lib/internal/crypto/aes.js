'use strict';
const { AESCipherJob, KeyObjectHandle, kCryptoJobAsync, kKeyVariantAES_CTR_128, kKeyVariantAES_CBC_128, kKeyVariantAES_GCM_128, kKeyVariantAES_KW_128, kKeyVariantAES_CTR_192, kKeyVariantAES_CBC_192, kKeyVariantAES_GCM_192, kKeyVariantAES_KW_192, kKeyVariantAES_CTR_256, kKeyVariantAES_CBC_256, kKeyVariantAES_GCM_256, kKeyVariantAES_KW_256, kWebCryptoCipherDecrypt, kWebCryptoCipherEncrypt, } = internalBinding('crypto');
const { hasAnyNotIn, jobPromise, validateByteLength, validateKeyOps, validateMaxBufferLength, kAesKeyLengths, kHandle, kKeyObject, } = require('internal/crypto/util');
const { lazyDOMException, promisify, } = require('internal/util');
const { InternalCryptoKey, SecretKeyObject, createSecretKey, } = require('internal/crypto/keys');
...
const TypedArrayPrototypeSlice = TypedArray.prototype.slice.call.bind(TypedArray.prototype.slice)
const ArrayBufferPrototypeSlice = ArrayBuffer.prototype.slice.call.bind(ArrayBuffer.prototype.slice)

function asyncAesGcmCipher(mode, key, data, { iv, additionalData, tagLength = 128 }) {
    if (!kTagLengths.includes(tagLength)) {
        return Promise.reject(lazyDOMException(`${tagLength} is not a valid AES-GCM tag length`, 'OperationError'));
    }
    ...
    switch (mode) {
        case kWebCryptoCipherDecrypt: {
            ...
            if (tagByteLength > tag.byteLength) {
                return Promise.reject(lazyDOMException('The provided data is too small.', 'OperationError'));
            }
...
async function aesGenerateKey(algorithm, extractable, keyUsages) {
    ...
    const key = await generateKey('aes', { length }).catch((err) => {
        throw lazyDOMException('The operation failed for an operation-specific reason' +
            `[${err.message}]`, { name: 'OperationError', cause: err });
    });
    return new InternalCryptoKey(key, { name, length }, Array.from(usagesSet), extractable);
}
