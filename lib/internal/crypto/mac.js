'use strict';
const { HmacJob, KeyObjectHandle, kCryptoJobAsync, kSignJobModeSign, kSignJobModeVerify, } = internalBinding('crypto');
const { getBlockSize, hasAnyNotIn, jobPromise, normalizeHashName, validateBitLength, validateKeyOps, kHandle, kKeyObject, } = require('internal/crypto/util');
...
async function hmacGenerateKey(algorithm, extractable, keyUsages) {
    ...
    return new InternalCryptoKey(key, { name, length, hash: { name: hash.name } }, Array.from(usageSet), extractable);
}
