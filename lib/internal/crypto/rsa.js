'use strict';
const { KeyObjectHandle, RSACipherJob, RSAKeyExportJob, SignJob, kCryptoJobAsync, kSignJobModeSign, kSignJobModeVerify, kKeyVariantRSA_SSA_PKCS1_v1_5, kKeyVariantRSA_PSS, kKeyVariantRSA_OAEP, kKeyTypePrivate, kWebCryptoCipherEncrypt, RSA_PKCS1_PSS_PADDING, } = internalBinding('crypto');
const { validateInt32, } = require('internal/validators');
const { bigIntArrayToUnsignedInt, getDigestSizeInBytes, getUsagesUnion, hasAnyNotIn, jobPromise, normalizeHashName, validateKeyOps, validateMaxBufferLength, kHandle, kKeyObject, } = require('internal/crypto/util');
const { lazyDOMException, promisify, } = require('internal/util');
const { InternalCryptoKey, PrivateKeyObject, PublicKeyObject, createPublicKey, createPrivateKey, } = require('internal/crypto/keys');
const { generateKeyPair: _generateKeyPair, } = require('internal/crypto/keygen');
const kRsaVariants = {
...
async function rsaKeyGenerate(algorithm, extractable, keyUsages) {
    const { name, modulusLength, publicExponent, hash, } = algorithm;
    ...
    const keypair = await generateKeyPair('rsa', {
        ...
        throw lazyDOMException('The operation failed for an operation-specific reason', { name: 'OperationError', cause: err });
    });
    const keyAlgorithm = {
        name,
        modulusLength,
    ...
    const publicKey = new InternalCryptoKey(keypair.publicKey, keyAlgorithm, publicUsages, true);
    const privateKey = new InternalCryptoKey(keypair.privateKey, keyAlgorithm, privateUsages, extractable);
...
async function rsaImportKey(format, keyData, algorithm, extractable, keyUsages) {
    ...
    switch (format) {
        ...
        case 'jwk': {
            ...
            verifyAcceptableRsaKeyUse(algorithm.name, keyData.d === undefined, usagesSet);
            ...
            if (keyData.alg !== undefined) {
                const hash = normalizeHashName(keyData.alg, normalizeHashName.kContextWebCrypto);
    ...
    const { modulusLength, publicExponent, } = keyObject[kHandle].keyDetail({});
...
async function rsaSignVerify(key, data, { saltLength }, signature) {
    ...
    return jobPromise(() => {
        if (key.algorithm.name === 'RSA-PSS') {
            validateInt32(saltLength, 'algorithm.saltLength', 0, Math.ceil((key.algorithm.modulusLength - 1) / 8) - getDigestSizeInBytes(key.algorithm.hash.name) - 2);
        }
        return new SignJob(kCryptoJobAsync, signature === undefined ? kSignJobModeSign : kSignJobModeVerify, key[kKeyObject][kHandle], undefined, undefined, undefined, data, normalizeHashName(key.algorithm.hash.name), saltLength, key.algorithm.name === 'RSA-PSS' ? RSA_PKCS1_PSS_PADDING : undefined, undefined, signature);
    });
}
module.exports = {
