    ...
    return (V, opts = kEmptyObject) => {
        ...
        if (opts.enforceRange) {
            ...
            if (x < lowerBound || x > upperBound) {
                throw makeException(`is outside the expected range of ${lowerBound} to ${upperBound}.`, { ...opts, code: 'ERR_OUT_OF_RANGE' });
            }
            return x;
        }
...
function requiredArguments(length, required, opts = kEmptyObject) {
    if (length < required) {
        throw makeException(`${required} argument${required === 1 ? '' : 's'} required, but only ${length} present.`, { ...opts, context: '', code: 'ERR_MISSING_ARGS' });
    }
}
    ...
    return function (V, opts = kEmptyObject) {
        ...
        for (const member of allMembers) {
            ...
            if (esMemberValue !== undefined) {
                ...
                throw makeException(`can not be converted to '${name}' because '${key}' is required in '${name}'.`, { ...opts, code: 'ERR_MISSING_OPTION' });
            }
        }
        return idlDict;
    };
