'use strict';
const { makeException, createEnumConverter, } = require('internal/webidl');
...
function toNumber(value, opts = kEmptyObject) {
    switch (typeof value) {
        case 'number':
            return value;
        case 'bigint':
            throw makeException('is a BigInt and cannot be converted to a number.', opts);
        case 'symbol':
            throw makeException('is a Symbol and cannot be converted to a number.', opts);
        default:
            return Number(value);
    }
}
    ...
    return (V, opts = kEmptyObject) => {
        ...
        if (opts.enforceRange) {
            ...
            if (x < lowerBound || x > upperBound) {
                throw makeException(`is outside the expected range of ${lowerBound} to ${upperBound}.`, { __proto__: null, ...opts, code: 'ERR_OUT_OF_RANGE' });
            }
            return x;
        }
...
converters.DOMString = function (V, opts = kEmptyObject) {
    if (typeof V === 'string') {
        ...
        throw makeException('is a Symbol and cannot be converted to a string.', opts);
    }
    return String(V);
};
...
function isNonSharedArrayBuffer(V) {
    return ArrayBuffer.prototype.isPrototypeOf(V);
}
function isSharedArrayBuffer(V) {
    // SharedArrayBuffers can be disabled with --no-harmony-sharedarraybuffer.
    if (SharedArrayBuffer !== undefined)
        return SharedArrayBuffer.prototype.isPrototypeOf(V);
    return false;
}
converters.Uint8Array = (V, opts = kEmptyObject) => {
    if (!ArrayBuffer.isView(V) ||
        V[Symbol.toStringTag] !== 'Uint8Array') {
        throw makeException('is not an Uint8Array object.', opts);
    }
    if (isSharedArrayBuffer(V.buffer)) {
        throw makeException('is a view on a SharedArrayBuffer, which is not allowed.', opts);
    }
    return V;
};
...
converters['sequence<DOMString>'] = createSequenceConverter(converters.DOMString);
function requiredArguments(length, required, opts = kEmptyObject) {
    if (length < required) {
        throw makeException(`${required} argument${required === 1 ? '' : 's'} required, but only ${length} present.`, { __proto__: null, ...opts, context: '', code: 'ERR_MISSING_ARGS' });
    }
}
    ...
    return function (V, opts = kEmptyObject) {
        ...
        for (const member of allMembers) {
            ...
            if (esMemberValue !== undefined) {
                const context = `'${key}' of '${name}'${opts.context ? ` (${opts.context})` : ''}`;
                ...
                throw makeException(`can not be converted to '${name}' because '${key}' is required in '${name}'.`, { __proto__: null, ...opts, code: 'ERR_MISSING_OPTION' });
            }
        }
        return idlDict;
    };
...
const dictRsaKeyGenParams = [
    ...
    {
        key: 'modulusLength',
        converter: (V, opts) => converters['unsigned long'](V, { ...opts, enforceRange: true }),
        required: true,
    },
    {
        key: 'publicExponent',
...
converters.RsaKeyGenParams = createDictionaryConverter('RsaKeyGenParams', dictRsaKeyGenParams);
converters.RsaHashedKeyGenParams = createDictionaryConverter('RsaHashedKeyGenParams', [
    ...dictRsaKeyGenParams,
    {
        key: 'hash',
...
converters.RsaHashedImportParams = createDictionaryConverter('RsaHashedImportParams', [
    ...dictAlgorithm,
    {
        key: 'hash',
        converter: converters.HashAlgorithmIdentifier,
...
converters.AesKeyGenParams = createDictionaryConverter('AesKeyGenParams', [
    ...
    {
        key: 'length',
        converter: (V, opts) => converters['unsigned short'](V, { ...opts, enforceRange: true }),
        required: true,
    },
]);
...
converters.RsaOtherPrimesInfo = createDictionaryConverter('RsaOtherPrimesInfo', [
    simpleDomStringKey('r'),
    simpleDomStringKey('d'),
    simpleDomStringKey('t'),
]);
converters['sequence<RsaOtherPrimesInfo>'] = createSequenceConverter(converters.RsaOtherPrimesInfo);
...
converters.AesDerivedKeyParams = createDictionaryConverter('AesDerivedKeyParams', [
    ...dictAlgorithm,
    {
        key: 'length',
        converter: (V, opts) => converters['unsigned short'](V, { ...opts, enforceRange: true }),
...
converters.AesGcmParams = createDictionaryConverter('AesGcmParams', [
    ...
    {
        key: 'tagLength',
        converter: (V, opts) => converters.octet(V, { ...opts, enforceRange: true }),
    },
    {
        key: 'additionalData',
        converter: converters.BufferSource,
...
converters.CryptoKey = createInterfaceConverter('CryptoKey', CryptoKey.prototype);
converters.EcdhKeyDeriveParams = createDictionaryConverter('EcdhKeyDeriveParams', [
    ...dictAlgorithm,
    {
        key: 'public',
