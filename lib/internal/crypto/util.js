'use strict';
const { getCiphers: _getCiphers, getCurves: _getCurves, getHashes: _getHashes, setEngine: _setEngine, secureHeapUsed: _secureHeapUsed, getCachedAliases, } = internalBinding('crypto');
const { getOptionValue } = require('internal/options');
const { crypto: { ENGINE_METHOD_ALL, }, } = internalBinding('constants');
const normalizeHashName = require('internal/crypto/hashnames');
const { hideStackFrames, codes: { ERR_CRYPTO_CUSTOM_ENGINE_NOT_SUPPORTED, ERR_CRYPTO_ENGINE_UNKNOWN, ERR_INVALID_ARG_TYPE, ERR_INVALID_ARG_VALUE, ERR_OUT_OF_RANGE, }, } = require('internal/errors');
const { validateArray, validateNumber, validateString, } = require('internal/validators');
const { Buffer } = require('buffer');
const { cachedResult, filterDuplicateStrings, lazyDOMException, } = require('internal/util');
const { namespace: { isBuildingSnapshot, addSerializeCallback, }, } = require('internal/v8/startup_snapshot');
const { isDataView, isArrayBufferView, isAnyArrayBuffer, } = require('internal/util/types');
...
const getArrayBufferOrView = hideStackFrames((buffer, name, encoding) => {
    ...
    if (!isArrayBufferView(buffer)) {
        throw new ERR_INVALID_ARG_TYPE.HideStackFramesError(name, [
            'string',
            'ArrayBuffer',
            'Buffer',
            'TypedArray',
...
function normalizeAlgorithm(algorithm, op) {
    ...
    for (const key in registeredAlgorithms) {
        if (!Object.hasOwn(registeredAlgorithms, key)) {
            continue;
        }
    ...
    for (let i = 0; i < dictKeys.length; i++) {
        ...
        if (idlType === 'BufferSource' && idlValue) {
            ...
            normalizedAlgorithm[member] = new Uint8Array(isView ? getDataViewOrTypedArrayBuffer(idlValue) : idlValue, isView ? getDataViewOrTypedArrayByteOffset(idlValue) : 0, isView ? getDataViewOrTypedArrayByteLength(idlValue) : idlValue.byteLength).slice();
        }
...
function getDataViewOrTypedArrayBuffer(V) {
    return isDataView(V) ? V.buffer : V.buffer;
}
function getDataViewOrTypedArrayByteOffset(V) {
    return isDataView(V) ? V.byteOffset : V.byteOffset;
}
function getDataViewOrTypedArrayByteLength(V) {
    return isDataView(V) ? V.byteLength : V.byteLength;
}
...
function onDone(resolve, reject, err, result) {
    if (err) {
        return reject(lazyDOMException('The operation failed for an operation-specific reason', { name: 'OperationError', cause: err }));
    }
    resolve(result);
}
function jobPromise(getJob) {
    return new Promise((resolve, reject) => {
        try {
            const job = getJob();
            job.ondone = onDone.bind(job, resolve, reject);
            job.run();
        }
        catch (err) {
