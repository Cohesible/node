...
const { ERR_WORKER_NOT_RUNNING, ERR_WORKER_PATH, ERR_WORKER_UNSERIALIZABLE_ERROR, ERR_WORKER_INVALID_EXEC_ARGV, ERR_INVALID_ARG_TYPE, ERR_INVALID_ARG_VALUE, } = errorCodes;
const { getOptionValue } = require('internal/options');
const workerIo = require('internal/worker/io');
const { drainMessagePort, receiveMessageOnPort, MessageChannel, messageTypes, kPort, kIncrementsPortRef, kWaitingStreams, kStdioWantsMoreDataCallback, setupPortReferencing, ReadableWorkerStdio, WritableWorkerStdio, } = workerIo;
...
const { ownsProcessState, isMainThread, resourceLimits: resourceLimitsRaw, threadId, Worker: WorkerImpl, kMaxYoungGenerationSizeMb, kMaxOldGenerationSizeMb, kCodeRangeSizeMb, kStackSizeMb, kTotalResourceLimitCount, } = internalBinding('worker');
...
class Worker extends EventEmitter {
    constructor(filename, options = kEmptyObject) {
        ...
        if (typeof options.env === 'object' && options.env !== null) {
            env = CreateNullProtoObject();
            Object.entries(options.env).forEach(({ 0: key, 1: value }) => { env[key] = `${value}`; });
        }
        ...
        this[kHandle] = new WorkerImpl(url, env === process.env ? null : env, options.execArgv, parseResourceLimits(options.resourceLimits), !!(options.trackUnmanagedFds ?? true), isInternal, name);
        ...
        ['message', 'messageerror'].forEach((event) => {
        ...
        this[kPort].postMessage({
            ...
            environmentData,
            publicPort: port2,
            manifestURL: null, // Dead code
            manifestSrc: null, // Dead code
    ...
    terminate(callback) {
        ...
        if (typeof callback === 'function') {
            process.emitWarning('Passing a callback to worker.terminate() is deprecated. ' +
                'It returns a Promise instead.', 'DeprecationWarning', 'DEP0132');
    ...
    getHeapSnapshot(options) {
        const { HeapSnapshotStream, getHeapSnapshotOptions, } = require('internal/heap_utils');
...
function parseResourceLimits(obj) {
    ...
    Uint8Array.prototype.fill.call(ret, -1);
